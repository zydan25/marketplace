...
export type StoreOrder = { id:number;orderCode:string;status:OrderStatus;statusLabel:string;totalAmount:number;createdAt:string;items:{id:number;productId:number;productName:string;color:string;size:string;unitPrice:number;quantity:number;sku:string;imageUrl?:string|null}[]; customer?:{name:string;phone:string;governorate?:string}; messages?:Array<{id:number;senderRole:"customer"|"admin"|"system";body:string|null;imageUrl:string|null;createdAt?:string}> };
...
export async function getOrder(id:number){
  const order=normalizeOrder(await djangoApi<DjangoOrder>(`/api/orders/${id}/`));
  try {
    const chats=await ensureOrderChats(id);
    const first=chats[0];
    order.messages=(first?.messages??[]).map(message=>({id:message.id,senderRole:"customer" as const,body:message.body,imageUrl:message.attachment_url,createdAt:message.created_at}));
  } catch { order.messages=[]; }
  return order;
}
