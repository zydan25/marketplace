...
class LaunchOrderViewSet(SecureOrderV2ViewSet):
    ...
    def _reprice_vendor_shipping(self, order):
        ...
        discount_split = self._distribute_amount(
            {item.id: {"subtotal": Decimal(item.subtotal)} for item in vendor_orders},
            discount_total,
        )
        ...
        for vendor_order in vendor_orders:
            ...
            if city:
                fee = (
                    VendorCityShipping.objects.filter(vendor_id=vendor_order.vendor_id, city_id=city.id, is_active=True)
                    .values_list("fee", flat=True)
                    .first()
                    or Decimal("0.00")
                )
            vendor_order.shipping_fee = fee
            vendor_order.discount = discount_split.get(vendor_order.id, Decimal("0.00"))
            vendor_order.commission = sum(
                (Decimal(link.order_item.commission) for link in vendor_order.items.select_related("order_item").all()),
                Decimal("0.00"),
            )
            vendor_order.total = max(Decimal("0.00"), vendor_order.subtotal - vendor_order.discount + fee)
            vendor_order.vendor_net = max(
                Decimal("0.00"),
                vendor_order.subtotal - vendor_order.discount - vendor_order.commission + fee,
            )
    ...
    def _refresh_vendor_finance(self, order):
        for vendor_order in order.vendor_orders.select_for_update().all():
            ...
            vendor_order.total = max(
                Decimal("0.00"), vendor_order.subtotal - vendor_order.discount + vendor_order.shipping_fee
            )
            vendor_order.vendor_net = max(
                Decimal("0.00"),
                vendor_order.subtotal - vendor_order.discount - vendor_order.commission + vendor_order.shipping_fee,
            )
    ...
    def create(self, request, *args, **kwargs):
        ...
        for vendor_order in order.vendor_orders.select_for_update().all():
            ...
            self._notify(
                ...
                "طلب جديد",
                f"وصل طلب جديد {order.order_number}. المبلغ محجوز حتى استلام العميل واعتماد العملية.",
            )
            ...
            if pay > 0:
                ...
                VendorLedgerEntry.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "vendor_order": payout.vendor_order,
