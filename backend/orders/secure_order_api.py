...
from catalog.services import PricingEngine
from communication.models import Notification, OrderChat
...
class SecureOrderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = (
            ...
            .prefetch_related("items", "items__vendor", "items__product", "vendor_orders", "payment", "coupon_redemption")
            .order_by("-created_at")
        )
    ...
    def _distribute_amount(groups, amount):
        ...
        if not groups or amount <= 0:
            return {vendor_id: Decimal("0.00") for vendor_id in groups}
        subtotal = sum((data["subtotal"] for data in groups.values()), Decimal("0.00")) or Decimal("1.00")
        result = {}
        allocated = Decimal("0.00")
        vendor_ids = list(groups)
        for vendor_id in vendor_ids[:-1]:
            share = (amount * groups[vendor_id]["subtotal"] / subtotal).quantize(Decimal("0.01"))
            result[vendor_id] = share
            allocated += share
        result[vendor_ids[-1]] = amount - allocated
        return result
    ...
    def _sync_parent_status(order):
        ...
        if not statuses:
            return
        if all(value == "delivered" for value in statuses):
            parent = Order.Status.DELIVERED
        elif all(value == "cancelled" for value in statuses):
            parent = Order.Status.CANCELLED
        elif any(value in {"delivered", "cancelled"} for value in statuses):
            parent = Order.Status.PARTIALLY_FULFILLED
        elif any(value == "shipped" for value in statuses):
            parent = Order.Status.SHIPPED
        elif any(value == "processing" for value in statuses):
            parent = Order.Status.PROCESSING
        elif any(value == "confirmed" for value in statuses):
    ...
    def _accrue_vendor_sale(vendor_order):
        ...
        before = previous.balance_after if previous else Decimal("0.00")
        VendorLedgerEntry.objects.create(
            ...
            balance_after=before + vendor_order.vendor_net,
            currency=vendor_order.currency,
            reference=reference,
            metadata={"source": "vendor_order_delivered"},
        )
    ...
    def _commit_vendor_order_inventory(vendor_order):
        ...
        SecureOrderViewSet._accrue_vendor_sale(vendor_order)

    @staticmethod
    def _resolve_coupon(code, user, subtotal):
        if not code:
            return None, Decimal("0.00")
        coupon = Coupon.objects.select_for_update().filter(code__iexact=code.strip()).first()
        if not coupon or not coupon.is_active:
            raise ValidationError({"coupon_code": "الكوبون غير صالح أو غير نشط"})
        now = timezone.now()
        if coupon.starts_at and now < coupon.starts_at:
            raise ValidationError({"coupon_code": "الكوبون لم يبدأ بعد"})
        if coupon.ends_at and now > coupon.ends_at:
            raise ValidationError({"coupon_code": "انتهت صلاحية الكوبون"})
        if coupon.usage_limit is not None and coupon.used_count >= coupon.usage_limit:
            raise ValidationError({"coupon_code": "استُنفدت مرات استخدام الكوبون"})
        if coupon.assigned_to.exists() and not coupon.assigned_to.filter(pk=user.pk).exists():
            raise ValidationError({"coupon_code": "هذا الكوبون غير متاح لهذا الحساب"})
        if subtotal < coupon.minimum_order:
            raise ValidationError({"coupon_code": f"الحد الأدنى للطلب هو {coupon.minimum_order}"})
        if coupon.discount_percent and coupon.discount_amount:
            raise ValidationError({"coupon_code": "إعداد الكوبون غير صالح: اختر نسبة أو مبلغًا ثابتًا فقط"})
        discount = (
            subtotal * coupon.discount_percent / Decimal("100")
    ...
    def create(self, request, *args, **kwargs):
        ...
        requires_reservation = payment_method != "cash_on_delivery"
        ...
        for row in rows:
            ...
            product = (
                ...
                .get(pk=product_id, is_published=True, vendor__status="active")
            )
            variant = None
            if row.get("variant_id") not in (None, ""):
                try:
                    variant = ProductVariant.objects.select_for_update().get(
                        pk=int(row["variant_id"]), product=product, is_active=True
                    )
            ...
            pricing = PricingEngine.calculate(product, city, quantity, variant)
            unit_price = (
                variant.price_override
                if variant and variant.price_override is not None
                else pricing["unit_final_price"]
            )
        ...
        coupon, coupon_discount = self._resolve_coupon(
            str(request.data.get("coupon_code", "")).strip(), request.user, subtotal
        )
        shipping_fee = city.shipping_fee if city else Decimal("0.00")
        total = max(Decimal("0.00"), subtotal - coupon_discount + shipping_fee)
        order = Order.objects.create(
            ...
            metadata={"pricing_source": "server", "api_version": "v2", "coupon_code": coupon.code if coupon else None},
        )
        if coupon:
            coupon.used_count += 1
            coupon.save(update_fields=["used_count", "updated_at"])
            CouponRedemption.objects.create(
                coupon=coupon, order=order, user=request.user,
                code_snapshot=coupon.code, discount_amount=coupon_discount, currency=currency,
            )

        shipping_by_vendor = self._distribute_amount(groups, shipping_fee)
        discount_by_vendor = self._distribute_amount(groups, coupon_discount)
        for vendor_id, group in groups.items():
            vendor = group["vendor"]
            vendor_subtotal = group["subtotal"]
            vendor_discount = discount_by_vendor[vendor_id]
            vendor_shipping = shipping_by_vendor[vendor_id]
            vendor_total = max(Decimal("0.00"), vendor_subtotal - vendor_discount + vendor_shipping)
            ...
            vendor_order = VendorOrder.objects.create(
                ...
                vendor_net=max(Decimal("0.00"), vendor_subtotal - commission + vendor_shipping),
                currency=currency,
            )
            ...
            OrderChat.objects.create(
                ...
                vendor=vendor,
                customer=request.user,
                subject=f"محادثة الطلب {order.order_number}",
            )
            for product, variant, quantity, row, unit_price, line_total in group["items"]:
                ...
                order_item = OrderItem.objects.create(
                    ...
                    vendor_net=max(Decimal("0.00"), line_total - item_commission),
                )
                VendorOrderItem.objects.create(vendor_order=vendor_order, order_item=order_item)
                reservation_status = InventoryReservation.Status.ACTIVE if requires_reservation else InventoryReservation.Status.COMMITTED
                ...
                InventoryReservation.objects.create(
                    ...
                    expires_at=timezone.now() + timedelta(minutes=30) if requires_reservation else timezone.now(),
                )
    ...
    def update_status(self, request, pk=None):
        ...
        if getattr(user, "role", None) == "vendor":
            ...
            if new_status == "delivered" and old_status != "delivered":
                ...
                if order.payment_method == "cash_on_delivery":
                    all_delivered = not order.vendor_orders.exclude(pk=vendor_order.pk).exclude(status="delivered").exists()
                    if all_delivered:
        ...
        if new_status == Order.Status.CANCELLED:
            for reservation in order.inventory_reservations.select_for_update().filter(
                status__in=[InventoryReservation.Status.ACTIVE, InventoryReservation.Status.COMMITTED]
            ):
                if reservation.variant_id:
