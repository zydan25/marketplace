...
class UnifiedWalletTests(TestCase):
    ...
    def test_transfer_is_ledger_backed_and_idempotent(self):
        ...
        first = transfer_between_users(
            self.sender, self.receiver, Decimal("250"), "YER",
            source_type="transfer", source_id="test:1", idempotency_key="transfer-test-001", created_by=self.sender,
        )
    ...
    def test_transfer_rejects_reuse_of_key_for_different_amount(self):
        ensure_legacy_customer_opening(self.sender, Decimal("1000"), "YER")
        transfer_between_users(
            self.sender, self.receiver, Decimal("100"), "YER",
            source_type="transfer", source_id="test:key", idempotency_key="same-key", created_by=self.sender,
        )
        with self.assertRaises(ValueError):
            transfer_between_users(
                self.sender, self.receiver, Decimal("101"), "YER",
                source_type="transfer", source_id="test:key", idempotency_key="same-key", created_by=self.sender,
            )
    ...
    def test_legacy_gift_creates_accounting_journal(self):
        ...
        self.assertEqual(wallet_balance(ensure_wallet(self.sender, AccountingWallet.Kinds.CUSTOMER, "YER")), Decimal("875.00"))
        self.assertEqual(wallet_balance(ensure_wallet(self.receiver, AccountingWallet.Kinds.CUSTOMER, "YER")), Decimal("125.00"))
    ...
    def test_secure_service_request_requires_idempotency(self):
        ...
        service = Service.objects.create(
            category=category, name="خدمة مدفوعة", slug="paid-service", code="PAID_TEST",
            service_kind=Service.ServiceKinds.PURCHASE, requires_balance=True,
            pricing_mode=Service.PricingModes.ITEM, currency="YER",
        )
        service.fields.create(key="mobile", label="الهاتف", required=True, validation={"min_length": 9, "max_length": 9})
        item = ServiceOption.objects.create(service=service, name="عنصر", external_code="X", provider_num="1", price=100)
        client = APIClient()
        client.force_authenticate(self.sender)
        response = client.post(
            "/api/v2/services/requests/",
            {"service_id": service.pk, "item_type": "service_options", "item_id": item.pk, "payload": {"mobile": self.sender.phone}},
            format="json",
        )
        self.assertEqual(response.status_code, 400)
        self.assertIn("idempotency_key", response.data)

    def test_public_catalog_does_not_expose_provider_credentials_or_ids(self):
        main = MainServiceCategory.objects.create(name="كتالوج", slug="catalog-tests")
        category = ServiceCategory.objects.create(main_category=main, name="ألعاب", slug="games")
        service = Service.objects.create(category=category, name="لعبة", slug="game", code="TEST_GAME", pricing_mode=Service.PricingModes.ITEM)
        service.fields.create(key="mobile", label="الهاتف", required=True)
        ServiceOption.objects.create(service=service, name="وحدة", external_code="SECRET-CODE", provider_num="999", price=50, metadata={"uniqcode": "SECRET-UNIQ", "provider_password": "DO_NOT_LEAK", "quota": 100})
        client = APIClient()
        client.force_authenticate(self.sender)
        response = client.get("/api/v2/services/catalog/")
        self.assertEqual(response.status_code, 200)
        text = str(response.data)
        self.assertNotIn("SECRET-CODE", text)
        self.assertNotIn("SECRET-UNIQ", text)
