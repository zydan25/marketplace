...
from .models import VendorLedgerEntry, VendorPayout, Wallet, WalletTransaction
...
class WalletTransactionDomainSerializer(serializers.ModelSerializer):
    class Meta:
        model = WalletTransaction
        fields = "__all__"
        read_only_fields = tuple(field.name for field in WalletTransaction._meta.fields)
...
class WalletViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def admin_adjust(self, request, pk=None):
        ...
        transaction_type = request.data.get("transaction_type", WalletTransaction.Types.ADJUSTMENT)
        ...
        if new_balance < 0:
            raise ValidationError({"amount": "لا يمكن أن يصبح الرصيد سالبًا"})
        ...
        entry = WalletTransaction.objects.create(
            ...
            metadata={
                "document_type": request.data.get("document_type", "receipt"),
                "issued_by": getattr(request.user, "phone", ""),
            },
        )
        return Response(
            {
                "wallet": WalletDomainSerializer(wallet).data,
                "transaction": WalletTransactionDomainSerializer(entry).data,
            },
...
class VendorFinanceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def _summary_for_vendor(self, vendor):
        ...
        paid_amount = VendorPayout.objects.filter(
            vendor=vendor, status="paid", vendor_order__isnull=True, order__isnull=True
        ).aggregate(v=Sum("amount"))["v"] or Decimal("0")
        pending_amount = VendorPayout.objects.filter(
            vendor=vendor, status__in=["pending", "approved"], vendor_order__isnull=True, order__isnull=True
        ).aggregate(v=Sum("amount"))["v"] or Decimal("0")
        wallet, _ = Wallet.objects.get_or_create(
            user=vendor.owner,
            defaults={"currency": ledger.order_by("-id").values_list("currency", flat=True).first() or "YER"},
        )
    ...
    def request_payout(self, request):
        ...
        if wallet.is_locked:
            raise PermissionDenied("المحفظة مقفلة")
        pending = VendorPayout.objects.select_for_update().filter(
            vendor=vendor, status__in=["pending", "approved"], vendor_order__isnull=True, order__isnull=True
        ).aggregate(v=Sum("amount"))["v"] or Decimal("0")
    ...
    def admin_pay(self, request, pk=None):
        ...
        payout.note = str(request.data.get("note") or payout.note or "").strip()
        payout.save(update_fields=["status", "note", "updated_at"])
        return Response({"payout": VendorPayoutDomainSerializer(payout).data, "wallet_balance": str(wallet.balance), "currency": wallet.currency})
