...
from .models import MainServiceCategory, ProviderConnection, Service, ServiceCategory, ServiceDistribution
...
class ServiceAdminV4Tests(TestCase):
    def setUp(self):
        User = get_user_model()
        self.admin = User.objects.create_user(username="admin-v4", password="pass", is_staff=True, role="admin")
    ...
    def test_admin_service_page_renders(self):
        ...
        self.assertEqual(client.login(username="admin-v4", password="pass"), True)
        response = client.get("/api/admin/dashboard/services/services/")
