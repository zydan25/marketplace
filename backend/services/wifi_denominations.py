...
class WifiDenomination(models.Model):
    ...
    denomination_number = models.CharField(max_length=80, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.denomination_number:
            self.denomination_number = f"W{self.network_id}-{uuid4().hex[:10].upper()}"
