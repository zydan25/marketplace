...
def dashboard(request):
        ...
        try:
            with transaction.atomic():
                if action == "main":
                    ...
                    service = get_object_or_404(Service, pk=request.POST.get("service"))
                    TelecomDenomination.objects.update_or_create(service=service, external_code=_required(request.POST, "external_code", "الكود الخارجي"), defaults={"name": _required(request.POST, "name", "الاسم"), "face_value": Decimal(request.POST["face_value"]), "sale_price": Decimal(request.POST["sale_price"]), "payment_type": request.POST.get("payment_type", ""), "line_type": request.POST.get("line_type", ""), "metadata": _json(request.POST.get("metadata"), {})})
                elif action == "telecom_plan":
                    service = get_object_or_404(Service, pk=request.POST.get("service"))
                    TelecomPlan.objects.update_or_create(service=service, external_code=_required(request.POST, "external_code", "الكود الخارجي"), defaults={"name": _required(request.POST, "name", "الاسم"), "price": Decimal(request.POST["price"]), "quota": Decimal(request.POST["quota"]) if request.POST.get("quota") else None, "quota_unit": request.POST.get("quota_unit", ""), "validity_days": int(request.POST["validity_days"]) if request.POST.get("validity_days") else None, "payment_type": request.POST.get("payment_type", ""), "line_type": request.POST.get("line_type", ""), "metadata": _json(request.POST.get("metadata"), {})})
                elif action == "game":
                    service = get_object_or_404(Service, pk=request.POST.get("service"))
                    GameProduct.objects.update_or_create(service=service, external_code=_required(request.POST, "external_code", "الكود الخارجي"), defaults={"name": _required(request.POST, "name", "الاسم"), "price": Decimal(request.POST["price"]), "currency": request.POST.get("currency", "YER"), "metadata": _json(request.POST.get("metadata"), {})})
                elif action == "digital":
                    service = get_object_or_404(Service, pk=request.POST.get("service"))
                    ...
                    model = model_map[request.POST.get("model")]
    ...
    return render(request, "services/dashboard.html", {
        "mains": MainServiceCategory.objects.all(), "categories": ServiceCategory.objects.select_related("main_category", "parent").all(),
        "services": services, "fields": ServiceField.objects.select_related("service"), "providers": ProviderConnection.objects.all(), "links": ProviderLink.objects.select_related("provider"),
        "distributions": ServiceDistribution.objects.select_related("service", "provider_link"), "transactions": ServiceTransaction.objects.select_related("service", "customer").order_by("-created_at")[:30],
        "tasks": ServiceTask.objects.select_related("transaction__service", "provider_link").order_by("-id")[:40], "plans": TelecomPlan.objects.select_related("service"), "denoms": TelecomDenomination.objects.select_related("service"),
        "games": GameProduct.objects.select_related("service"), "digital": DigitalProduct.objects.select_related("service"),
        "stats": {"main": MainServiceCategory.objects.filter(is_active=True).count(), "categories": ServiceCategory.objects.filter(is_active=True).count(), "services": Service.objects.filter(is_active=True).count(), "providers": ProviderConnection.objects.filter(is_active=True).count(), "queued": ServiceTask.objects.filter(status__in=[ServiceTask.Statuses.QUEUED, ServiceTask.Statuses.RETRY]).count()},
    })
