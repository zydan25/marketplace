...
class Command(BaseCommand):
    help = "يفحص كتالوج الخدمات للتأكد من سلامة العناصر المدفوعة وأسعارها."
    ...
    def handle(self, *args, **options):
        ...
        for model_name, model in ITEM_MODELS.items():
            for item in model.objects.filter(is_active=True, service_id__in=purchase_codes).select_related("service").iterator():
                ...
                if meta.get("purchaseable", True) is False or meta.get("requires_balance", True) is False:
                    continue
                ...
                if price <= 0:
                    errors.append(
                        f"{model_name}#{item.pk} service={item.service.code}: purchaseable with non-positive price"
                    )

        # Provider routing has its own dedicated audit command. The catalog
        # audit must remain useful on a clean CI database without provider
        # credentials or a configured external connection.
