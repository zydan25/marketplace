...
MAIN = {"payments": ("التسديدات", "التسديدات"), "games": ("الألعاب", "الألعاب"), "digital": ("البرامج والبطاقات", "البرامج-والبطاقات")}
...
def provision_links(connection, services):
    ...
    for category_slug, name, service_code, pricing, link_key in SERVICES:
        ServiceDistribution.objects.update_or_create(service=services[service_code], provider_link=result[link_key], defaults={"priority": 100, "is_active": True, "conditions": {}})
    return result
