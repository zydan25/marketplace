...
def ensure_service_accounts():
    ...
    pending, _ = Account.objects.get_or_create(
        code="400901",
        defaults={"name": "تسويات الخدمات المعلقة", "parent": chart["suppliers"], "account_type": Account.Types.LIABILITY, "normal_side": Account.NormalSides.CREDIT, "is_group": False, "metadata": {"domain": "services", "purpose": "customer_reservation"}},
    )
    revenue, _ = Account.objects.get_or_create(
        code="600101",
        defaults={"name": "إيرادات الخدمات", "parent": chart["income"], "account_type": Account.Types.INCOME, "normal_side": Account.NormalSides.CREDIT, "is_group": False, "metadata": {"domain": "services"}},
    )
...
def settle_service(service_transaction):
    ...
    return post_entry(
        ...
        source_type="service_settlement", source_id=service_transaction.id, idempotency_key=key,
