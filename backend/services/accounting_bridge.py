...
def ensure_service_accounts():
    ...
    pending, _ = Account.objects.get_or_create(
        code="400901",
        defaults={
            "name": "تسويات الخدمات المعلقة",
            "parent": chart["suppliers"],
            "account_type": Account.Types.LIABILITY,
            "normal_side": Account.NormalSides.CREDIT,
            "is_group": False,
            "metadata": {"domain": "services", "purpose": "customer_reservation"},
        },
    )
...
def reserve_service_funds(service_transaction):
    ...
    customer_wallet = ensure_wallet(service_transaction.customer, "customer", service_transaction.currency)
    ...
    return post_entry(
        ...
        source_type="service_reservation", source_id=service_transaction.id,
        idempotency_key=key, created_by=service_transaction.customer,
...
def settle_service(service_transaction):
    ...
    if JournalEntry.objects.filter(idempotency_key=key).exists():
        return JournalEntry.objects.get(idempotency_key=key)
    return post_entry(
        f"تسوية خدمة ناجحة {service_transaction.service.code}",
        [{"account": accounts["pending"], "debit": amount}, {"account": accounts["revenue"], "credit": amount}],
        source_type="service_settlement", source_id=service_transaction.id,
        idempotency_key=key, metadata={"service_transaction": str(service_transaction.id), "currency": service_transaction.currency},
    )
...
def refund_service(service_transaction):
    ...
    if JournalEntry.objects.filter(idempotency_key=key).exists():
        return JournalEntry.objects.get(idempotency_key=key)
    return post_entry(
        f"إعادة حجز خدمة فاشلة {service_transaction.service.code}",
