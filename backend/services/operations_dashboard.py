...
def balances_dashboard(request):
    wallets = list(Wallet.objects.select_related("owner", "account").filter(is_active=True).order_by("-updated_at")[:150])
    for wallet in wallets:
        wallet.current_balance = account_balance(wallet.account)

    key_accounts = list(Account.objects.filter(is_active=True, code__in=["100001", "400901", "600001", "600101"]).order_by("code"))
    for account in key_accounts:
        account.current_balance = account_balance(account)

    journal_entries = list(JournalEntry.objects.select_related("created_by").prefetch_related("lines__account").order_by("-id")[:100])
    ...
    grouped = {}
    for wallet in wallets:
        grouped.setdefault(wallet.currency, []).append(wallet)
    for currency, currency_wallets in grouped.items():
        values = {kind: Decimal("0.00") for kind, _ in Wallet.Kinds.choices}
        for wallet in currency_wallets:
            values[wallet.kind] += wallet.current_balance
        currency_summaries.append({
            "currency": currency,
            "customer": values[Wallet.Kinds.CUSTOMER],
            "vendor_pending": values[Wallet.Kinds.VENDOR_PENDING],
            "vendor_available": values[Wallet.Kinds.VENDOR_AVAILABLE],
            "withdrawal_hold": values[Wallet.Kinds.WITHDRAWAL_HOLD],
            "total": sum(values.values(), Decimal("0.00")),
        })

    return render(request, "services/balances.html", {
        "section": "balances",
        "wallets": wallets,
