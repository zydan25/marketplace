...
def _mark_manual_review(tx, *, code, message):
    with transaction.atomic():
        tx = ServiceTransaction.objects.select_for_update().get(pk=tx.pk)
        if tx.status in {ServiceTransaction.Status.SUCCESS, ServiceTransaction.Status.REFUNDED}:
            return tx
        tx.status = ServiceTransaction.Status.MANUAL_REVIEW
        tx.error_code = str(code or "MANUAL_REVIEW")
        tx.error_message = str(message or "")
...
def process_task(task_id=None):
    ...
    with transaction.atomic():
        # provider_link is nullable. Lock only ServiceTask rows; PostgreSQL then
        # doesn't try to FOR UPDATE the nullable outer-joined provider relation.
        qs = ServiceTask.objects.select_for_update(of=("self",)).select_related("transaction__service", "provider_link__provider")
    ...
    if result.pending:
        ...
        if task.kind == ServiceTask.Kinds.SUBMIT:
            ...
            delay = int((task.metadata or {}).get("poll_seconds", 8))
            task.status = ServiceTask.Statuses.RETRY
            task.available_at = timezone.now() + timedelta(seconds=max(5, min(120, delay)))
            task.last_error = result.description
            task.started_at = None
        else:
            ...
            task.finished_at = timezone.now()
            task.last_error = result.description
        task.save(update_fields=["status", "available_at", "finished_at", "last_error", "started_at"])
        return task
    ...
    if task.attempts < task.max_attempts:
        ...
        task.last_error = result.description
        task.started_at = None
        task.save(update_fields=["status", "available_at", "last_error", "started_at"])
        tx.status = ServiceTransaction.Status.PROCESSING
    ...
    task.finished_at = timezone.now()
    task.started_at = None
    task.save(update_fields=["status", "last_error", "finished_at", "started_at"])
    return task
