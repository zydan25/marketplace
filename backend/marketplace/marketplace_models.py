...
class Payment(models.Model):
    class Status(models.TextChoices):
        PENDING="pending","قيد الانتظار"; AUTHORIZED="authorized","مصرح"; PAID="paid","مدفوع"; FAILED="failed","فشل"; REFUNDED="refunded","مسترد"; PARTIALLY_REFUNDED="partially_refunded","مسترد جزئيًا"; CANCELLED="cancelled","ملغي"
    created_at=models.DateTimeField(auto_now_add=True); updated_at=models.DateTimeField(auto_now=True)
    order=models.OneToOneField("marketplace.Order",on_delete=models.PROTECT,related_name="payment")
    provider=models.CharField(max_length=60,default="manual"); method=models.CharField(max_length=60,default="cash_on_delivery")
    ...
    currency=models.CharField(max_length=6,default="YER"); status=models.CharField(max_length=30,choices=Status.choices,default=Status.PENDING)
    paid_at=models.DateTimeField(null=True,blank=True); metadata=models.JSONField(default=dict,blank=True)


class Shipment(models.Model):
    class Status(models.TextChoices):
        PENDING="pending","قيد الانتظار"; READY="ready","جاهز"; SHIPPED="shipped","تم الشحن"; IN_TRANSIT="in_transit","في الطريق"; DELIVERED="delivered","تم التسليم"; RETURNED="returned","مرتجع"; CANCELLED="cancelled","ملغي"
    created_at=models.DateTimeField(auto_now_add=True); updated_at=models.DateTimeField(auto_now=True)
    vendor_order=models.OneToOneField(VendorOrder,on_delete=models.PROTECT,related_name="shipment")
    carrier=models.CharField(max_length=120,blank=True); tracking_number=models.CharField(max_length=160,blank=True)
    status=models.CharField(max_length=30,choices=Status.choices,default=Status.PENDING); shipped_at=models.DateTimeField(null=True,blank=True); delivered_at=models.DateTimeField(null=True,blank=True); metadata=models.JSONField(default=dict,blank=True)


class InventoryReservation(models.Model):
    class Status(models.TextChoices):
        ACTIVE="active","نشط"; COMMITTED="committed","مثبت"; RELEASED="released","محرر"; EXPIRED="expired","منتهي"
    created_at=models.DateTimeField(auto_now_add=True); updated_at=models.DateTimeField(auto_now=True); expires_at=models.DateTimeField()
    order=models.ForeignKey("marketplace.Order",on_delete=models.CASCADE,related_name="inventory_reservations")
    order_item=models.ForeignKey("marketplace.OrderItem",on_delete=models.PROTECT,related_name="inventory_reservations",null=True,blank=True)
    ...
    quantity=models.PositiveIntegerField(); status=models.CharField(max_length=20,choices=Status.choices,default=Status.ACTIVE)
    class Meta:
...
class VendorLedgerEntry(models.Model):
    class Types(models.TextChoices):
        SALE="sale","بيع"; COMMISSION="commission","عمولة"; REFUND="refund","استرداد"; PAYOUT="payout","سحب"; ADJUSTMENT="adjustment","تسوية"
    ...
    entry_type=models.CharField(max_length=30,choices=Types.choices); amount=models.DecimalField(max_digits=14,decimal_places=2); balance_after=models.DecimalField(max_digits=14,decimal_places=2); currency=models.CharField(max_length=6,default="YER"); reference=models.CharField(max_length=160,unique=True); metadata=models.JSONField(default=dict,blank=True)
...
class CouponRedemption(models.Model):
    ...
    code_snapshot=models.CharField(max_length=50); discount_amount=models.DecimalField(max_digits=14,decimal_places=2,validators=[MinValueValidator(0)]); currency=models.CharField(max_length=6,default="YER")
    class Meta:
...
class VendorApplication(models.Model):
    class Status(models.TextChoices):
        PENDING="pending","قيد المراجعة"; APPROVED="approved","مقبول"; REJECTED="rejected","مرفوض"
    created_at=models.DateTimeField(auto_now_add=True); updated_at=models.DateTimeField(auto_now=True)
    applicant=models.OneToOneField(settings.AUTH_USER_MODEL,on_delete=models.CASCADE,related_name="vendor_application")
    store_name=models.CharField(max_length=180); description=models.TextField(blank=True); phone=models.CharField(max_length=32); address=models.CharField(max_length=255,blank=True)
    documents=models.JSONField(default=dict,blank=True); status=models.CharField(max_length=20,choices=Status.choices,default=Status.PENDING)
    review_note=models.TextField(blank=True); reviewed_by=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.SET_NULL,null=True,blank=True,related_name="vendor_applications_reviewed"); reviewed_at=models.DateTimeField(null=True,blank=True)
    class Meta:
        ordering=["-created_at"]
