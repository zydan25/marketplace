...
from .models import (Category, Conversation, Coupon, DesignTheme, Message, Notification, Order, OrderItem, Product, ProductImage, Referral, StorefrontSection, User, VendorPayout, VendorProfile, Wallet, WalletTransaction, PlatformLedgerEntry)
...
class MarketplaceUserAdmin(UserAdmin):
    list_display=("phone","get_full_name","role","governorate","is_active","date_joined"); list_filter=("role","is_active","is_phone_verified","governorate"); search_fields=("phone","first_name","middle_name","third_name","last_name","email"); fieldsets=UserAdmin.fieldsets+(("ملف السوق",{"fields":("phone","role","middle_name","third_name","governorate","avatar","is_phone_verified")}),)
...
class ProductAdmin(admin.ModelAdmin):
    list_display=("name","sku","vendor","effective_price_display","stock","reserved_stock","is_published","is_trending"); list_filter=("is_published","is_trending","currency","vendor"); search_fields=("name","sku","description","vendor__store_name"); filter_horizontal=("categories",)
...
class StorefrontMediaAdmin(admin.ModelAdmin):
    list_display=("name","vendor","is_active","target_url","updated_at"); list_filter=("is_active","vendor"); search_fields=("name","alt_text","target_url","vendor__store_name"); autocomplete_fields=("vendor",)
...
class WalletHoldAdmin(admin.ModelAdmin):
    list_display=("order","wallet","amount","released_amount","refunded_amount","status","created_at"); list_filter=("status",); search_fields=("order__order_number","wallet__user__phone"); readonly_fields=("wallet","order","amount","released_amount","refunded_amount","status","created_at","updated_at")
@admin.register(PlatformLedgerEntry)
class PlatformLedgerEntryAdmin(admin.ModelAdmin):
    list_display=("order","entry_type","amount","currency","reference","created_at"); list_filter=("entry_type","currency"); search_fields=("reference","order__order_number")
...
class ServiceSubmissionAdmin(admin.ModelAdmin):
    list_display=("reference","service","customer","amount","currency","status","created_at"); list_filter=("status","currency"); search_fields=("reference","service__name","customer__phone")
@admin.register(VendorCityShipping)
class VendorCityShippingAdmin(admin.ModelAdmin):
    list_display=("vendor","city","fee","is_active","updated_at"); list_filter=("is_active","city"); search_fields=("vendor__store_name","city__name")
@admin.register(MarketplaceOffice)
class MarketplaceOfficeAdmin(admin.ModelAdmin):
    list_display=("city","name","office_fee","phone","is_active","updated_at"); list_filter=("is_active","city"); search_fields=("city__name","name","phone")
    ...
    def pay_selected(self,request,queryset):
        with transaction.atomic():
            for payout in queryset.select_related("vendor").filter(status="approved"):
                ...
                VendorLedgerEntry.objects.create(vendor=payout.vendor,entry_type=VendorLedgerEntry.Types.PAYOUT,amount=-payout.amount,balance_after=before-payout.amount,currency=payout.currency,reference=f"PAYOUT-{payout.id}",metadata={"admin":request.user.phone}); payout.status="paid"; payout.save(update_fields=("status","updated_at"))
