...
from .models import Coupon, Notification, Order, OrderStatusHistory, Product, ProductVariant, VendorLedgerEntry, VendorPayout, Wallet, WalletTransaction
...
class LaunchOrderViewSet(SecureOrderV2ViewSet):
    ...
    def _reprice_vendor_shipping(self, order):
        ...
        subtotals = {item.id: Decimal(item.subtotal) for item in vendor_orders}
        ...
        for vendor_order in vendor_orders:
            ...
            vendor_order.save(update_fields=["shipping_fee", "discount", "total", "commission", "vendor_net", "updated_at"])
            total_shipping += fee
    ...
    def _refresh_vendor_finance(self, order):
        for vendor_order in order.vendor_orders.select_for_update().all():
            ...
            vendor_order.save(update_fields=["commission", "vendor_net", "total", "updated_at"])
    ...
    def update_pending(self, request, pk=None):
        ...
        for row in items:
            ...
            quantities[int(row["order_item_id"])] = quantity
        ...
        vendors = defaultdict(lambda: {"subtotal": Decimal("0.00"), "items": []})
        for item in current_items:
            ...
            subtotal += line_total
            vendors[item.vendor_id]["subtotal"] += line_total
            vendors[item.vendor_id]["items"].append(item)
            if variant:
            ...
            InventoryReservation.objects.create(
                order=order, order_item=item, variant=variant, product=None if variant else product,
                quantity=quantity, status=InventoryReservation.Status.ACTIVE, expires_at=timezone.now() + timedelta(minutes=30)
            )
        ...
        if coupon_code:
            ...
            if coupon and (coupon.minimum_order <= subtotal):
                if coupon.discount_percent:
                    order.discount = min((subtotal * coupon.discount_percent / Decimal("100")).quantize(Decimal("0.01")), subtotal)
                else:
                    order.discount = min(Decimal(coupon.discount_amount), subtotal)
        else:
    ...
    def order_view(self, request, pk=None):
        ...
        for item in payload["items"]:
            model_item = order.items.get(pk=item["id"])
            item_payload.append({
                **item,
                "product_url": f"/product/{item['product']}",
                "vendor_url": f"/store/{model_item.vendor.slug}",
                "image_url": request.build_absolute_uri(model_item.product.main_image.url) if model_item.product.main_image else None,
            })
    ...
    def confirm_received(self, request, pk=None):
        ...
        unresolved = [x for x in (escrow.get("disputes") or {}).values() if x.get("status") == "pending"]
        if unresolved:
    ...
    def reject_item(self, request, pk=None):
        ...
        if request.user.role == "customer":
            if order.customer_id != request.user.id:
                raise PermissionDenied("لا تملك هذا الطلب")
            if order.status != Order.Status.DELIVERED:
                raise ValidationError({"order": "يمكن فتح الاعتراض بعد تسليم الطلب."})
        elif request.user.role == "vendor":
            if item.vendor.owner_id != request.user.id:
    ...
    def reject_order(self, request, pk=None):
        ...
        if order.customer_id != request.user.id:
            raise PermissionDenied("لا تملك هذا الطلب")
        if order.status != Order.Status.DELIVERED or not self._is_escrow(order):
            raise ValidationError({"order": "يمكن رفض الطلب بعد تسليمه فقط."})
        reason = str(request.data.get("reason", "")).strip()
        if not reason:
            raise ValidationError({"reason": "سبب رفض الطلب مطلوب."})
        escrow = self._escrow(order)
        disputes = dict(escrow.get("disputes") or {})
    ...
    def admin_release(self, request, pk=None):
        ...
        unresolved = {int(key) for key, value in disputes.items() if value.get("status") == "pending" and key.isdigit()}
        ...
        for payout in VendorPayout.objects.select_for_update().filter(order=order, status="pending"):
            ...
            if payout.vendor_order_id:
                for link in VendorOrderItem.objects.filter(vendor_order_id=payout.vendor_order_id).select_related("order_item"):
                    if link.order_item_id in unresolved:
    ...
    def resolve_item_dispute(self, request, pk=None):
        ...
        if decision == "refund":
            ...
            self._wallet_credit(wallet, refund, WalletTransaction.Types.REFUND, f"DISPUTE-REFUND-{item.id}", f"استرداد قيمة القطعة من الطلب {order.order_number}", {"order_item_id": item.id})
            ...
            self._wallet_credit(vendor_wallet, amount, WalletTransaction.Types.REWARD, f"DISPUTE-RELEASE-{item.id}", f"إطلاق مستحق القطعة بعد حل الاعتراض {order.order_number}", {"order_item_id": item.id})
            payout = VendorPayout.objects.filter(order=order, vendor=item.vendor).first()
            if payout and not VendorOrderItem.objects.filter(vendor_order_id=payout.vendor_order_id, order_item_id__in=[int(k) for k, v in disputes.items() if v.get("status") == "pending"], vendor_order_id=payout.vendor_order_id).exists():
                payout.status = "paid"
                payout.save(update_fields=["status", "updated_at"])
    ...
    def update_status(self, request, pk=None):
        ...
        if user.role == "vendor" and self._is_escrow(order):
            ...
            if new_status == "shipped" and getattr(vendor_order, "shipment", None):
                shipment = vendor_order.shipment
            ...
            if new_status == "delivered" and old != "delivered":
                for link in vendor_order.items.select_related("order_item__product").all():
                    ...
                    if reservation:
                        ...
                        reservation.save(update_fields=["status", "updated_at"])
                    link.order_item.product.sold_count += link.order_item.quantity
                    link.order_item.product.save(update_fields=["sold_count", "updated_at"])
        ...
        if new_status == Order.Status.CANCELLED and self._is_escrow(order):
            ...
            if remaining > 0:
                wallet = Wallet.objects.select_for_update().get(user=order.customer)
                self._wallet_credit(wallet, remaining, WalletTransaction.Types.REFUND, f"ORDER-CANCEL-REFUND-{order.id}", f"استرداد قيمة الطلب الملغي {order.order_number}", {"order_id": order.id})
