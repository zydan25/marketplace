...
def _stores_context(request, form=None, edit_vendor=None, form_mode="create"):
    ...
    stats = {
        ...
        "products": sum(v.product_count for v in page.object_list),
    }
    ...
    return {
        ...
        "form_mode": form_mode,
        "store_form_open": bool(form and (form_mode == "create" or edit_vendor)),
        "control_stores_url": reverse("admin-control-stores"),
    }
...
def stores(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"تم حفظ المتجر «{vendor.store_name}» بنجاح.",
            )
