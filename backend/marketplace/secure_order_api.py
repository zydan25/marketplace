...
class SecureOrderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = Order.objects.select_related("customer").prefetch_related("items", "items__vendor", "items__product", "vendor_orders", "payment", "coupon_redemption")
    ...
    def _distribute_amount(groups, amount):
        ...
        result = {}; allocated = Decimal("0.00"); vendor_ids = list(groups)
        for vendor_id in vendor_ids[:-1]:
            share = (amount * groups[vendor_id]["subtotal"] / subtotal).quantize(Decimal("0.01")); result[vendor_id] = share; allocated += share
    ...
    def _sync_parent_status(order):
        ...
        if order.status != parent:
            old_status = order.status; order.status = parent; order.save(update_fields=["status", "updated_at"])
    ...
    def _accrue_vendor_sale(vendor_order):
        ...
        VendorLedgerEntry.objects.create(vendor=vendor_order.vendor, vendor_order=vendor_order, entry_type=VendorLedgerEntry.Types.SALE, amount=vendor_order.vendor_net, balance_after=balance_before + vendor_order.vendor_net, currency=vendor_order.currency, reference=reference, metadata={"source":"vendor_order_delivered"})

    @staticmethod
    def _commit_vendor_order_inventory(vendor_order):
        for link in vendor_order.items.select_related("order_item__product", "order_item__inventory_reservations").all():
            item = link.order_item
            reservation = InventoryReservation.objects.select_for_update().filter(order_item=item).exclude(status=InventoryReservation.Status.RELEASED).first()
            if reservation:
                ...
                elif reservation.status == InventoryReservation.Status.EXPIRED:
                    raise ValidationError({"order": "لا يمكن تسليم الطلب لأن حجز المخزون منتهي."})
            if item.product.sold_count < 0: item.product.sold_count = 0
            item.product.sold_count += item.quantity; item.product.save(update_fields=["sold_count","updated_at"])
        VendorPayout.objects.get_or_create(vendor_order=vendor_order, defaults={"vendor":vendor_order.vendor,"order":vendor_order.order,"amount":vendor_order.vendor_net,"currency":vendor_order.currency,"status":"pending","reference":f"PAYOUT-{vendor_order.id}"})
    ...
    def create(self, request, *args, **kwargs):
        ...
        for vendor_id,group in groups.items():
            ...
            vendor_order=VendorOrder.objects.create(order=order,vendor=vendor,order_number=f"{order.order_number}-{vendor_id}",subtotal=vendor_subtotal,shipping_fee=vendor_shipping,discount=vendor_discount,total=vendor_total,commission=commission,vendor_net=vendor_subtotal-commission+vendor_shipping,currency=currency); Shipment.objects.create(vendor_order=vendor_order)
    ...
    def update_status(self,request,pk=None):
        ...
        if user.role=="vendor":
            ...
            if new_status not in allowed: raise ValidationError({"status":"حالة التاجر غير صالحة"})
