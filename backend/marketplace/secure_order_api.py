...
class SecureOrderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = Order.objects.select_related("customer").prefetch_related(
            "items", "items__vendor", "items__product", "vendor_orders", "payment", "coupon_redemption"
        )
    ...
    def _accrue_vendor_sale(vendor_order):
        existing = VendorLedgerEntry.objects.filter(reference=f"SALE-{vendor_order.id}").first()
        if existing:
            return
        previous = VendorLedgerEntry.objects.filter(vendor=vendor_order.vendor, currency=vendor_order.currency).order_by("-id").first()
        ...
        VendorLedgerEntry.objects.create(
            ...
            balance_after=balance_before + vendor_order.vendor_net,
    ...
    def create(self, request, *args, **kwargs):
        ...
        for row in rows:
            ...
            if row.get("variant_id") not in (None, ""):
                try:
                    variant = ProductVariant.objects.select_for_update().get(id=int(row["variant_id"]), product=product, is_active=True)
        ...
        coupon, coupon_discount = self._resolve_coupon(str(request.data.get("coupon_code", "")).strip(), request.user, subtotal, currency)
        ...
        order = Order.objects.create(
            ...
            metadata={"pricing_source": "server", "api_version": "v1", "coupon_code": coupon.code if coupon else None},
        )
        if coupon:
            coupon.used_count += 1
        ...
        for vendor_id, group in groups.items():
            ...
            vendor_order = VendorOrder.objects.create(
                order=order, vendor=vendor, order_number=f"{order.order_number}-{vendor_id}",
                subtotal=vendor_subtotal, shipping_fee=vendor_shipping, discount=vendor_discount,
                total=vendor_total, commission=commission,
                vendor_net=vendor_subtotal - commission + vendor_shipping, currency=currency,
            )
            ...
            for product, variant, quantity, row, unit_price, line_total in group["items"]:
                ...
                order_item = OrderItem.objects.create(
                    ...
                    name_snapshot=product.name, sku_snapshot=variant.sku if variant else product.sku,
                    quantity=quantity, unit_price=unit_price,
                    color=variant.color if variant else str(row.get("color", "")),
                    size=variant.size if variant else str(row.get("size", "")),
                    vendor_total=line_total, commission=item_commission, vendor_net=line_total - item_commission,
                )
                ...
                InventoryReservation.objects.create(
                    order=order, order_item=order_item, variant=variant,
                    product=None if variant else product, quantity=quantity,
                    status=reservation_status,
                    expires_at=timezone.now() + timedelta(minutes=30) if requires_payment else timezone.now(),
                )

        Payment.objects.create(
            order=order,
            provider="manual" if payment_method == "cash_on_delivery" else payment_method,
            method=payment_method, amount=total, currency=currency,
            status=Payment.Status.PENDING, metadata={"source": "checkout"},
        )
        ...
        if user.role == "vendor":
            ...
            if new_status == "delivered" and old_status != "delivered":
                ...
                if getattr(vendor_order, "shipment", None):
                    vendor_order.shipment.status = Shipment.Status.DELIVERED
                    vendor_order.shipment.delivered_at = timezone.now()
                    vendor_order.shipment.save(update_fields=["status", "delivered_at", "updated_at"])
