...
def _product_context(request, form=None, edit_product=None, form_open=False):
    ...
    return {
        ...
        "stats": {
            ...
            "hidden": Product.objects.filter(is_published=False).count(),
...
def control_stores(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if is_ajax(request):
                return render_control_partial(request, "stores")
            return redirect(CONTROL_STORES)
    else:
...
def control_products(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if is_ajax(request):
                return render_control_partial(request, "products")
