...
def _store_context(request, form=None, edit_vendor=None, form_open=False):
    ...
    if q:
        qs = qs.filter(
            Q(store_name__icontains=q) | Q(slug__icontains=q) | Q(phone__icontains=q)
            | Q(owner__phone__icontains=q) | Q(owner__email__icontains=q)
    ...
    return {
        ...
        "form": form or VendorProfileForm(initial={"status": "active", "commission_percent": 10}),
...
def _product_context(request, form=None, edit_product=None, form_open=False):
    ...
    for value in Product.objects.values_list("colors", flat=True).iterator():
        if isinstance(value, list):
            colors.extend(str(v) for v in value if v)
    for value in Product.objects.values_list("sizes", flat=True).iterator():
        if isinstance(value, list):
            sizes.extend(str(v) for v in value if v)
    colors = list(dict.fromkeys(colors))[:80]
    sizes = list(dict.fromkeys(sizes))[:60]
    ...
    product_report = {
        "units_sold": sales.get("quantity") or 0,
        "revenue": sales.get("revenue") or 0,
        "orders": OrderItem.objects.filter(product=edit_product).values("order_id").distinct().count() if edit_product else 0,
        "rating": edit_product.rating if edit_product else 0,
        "reviews": edit_product.reviews_count if edit_product else 0,
        "stock": edit_product.available_stock if edit_product else 0,
    }
    return {
        "page": page,
        ...
        "size_options": sizes,
        "selected_colors": _json_list(form, edit_product, "colors"),
        "selected_sizes": _json_list(form, edit_product, "sizes"),
        "selected_hashtags": _json_list(form, edit_product, "hashtags"),
        "form": form,
...
def control_stores(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            vendor = form.save()
            messages.success(request, f"تم حفظ المتجر «{vendor.store_name}».")
...
def control_store_status(request, vendor_id, status):
    ...
    messages.success(request, f"تم تحديث حالة متجر «{vendor.store_name}».")
    return render_control_partial(request, "stores") if is_ajax(request) else redirect(CONTROL_STORES)
...
def control_store_delete(request, vendor_id):
    ...
    if vendor.products.exists() or vendor.vendor_orders.exists() or vendor.order_items.exists():
        messages.error(request, "لا يمكن حذف متجر مرتبط بمنتجات أو طلبات. استخدم الإيقاف بدل الحذف.")
    else:
        ...
        try:
            ...
            messages.error(request, "تعذر حذف المتجر بسبب ارتباط محمي في النظام.")
    return render_control_partial(request, "stores") if is_ajax(request) else redirect(CONTROL_STORES)
...
def control_store_report(request, vendor_id):
    ...
    orders = vendor.vendor_orders.all()
    sales = orders.aggregate(gross=Sum("total"), commission=Sum("commission"), net=Sum("vendor_net"))
    products = vendor.products.annotate(sales_units=Sum("order_items__quantity"), sales_revenue=Sum("order_items__vendor_total")).order_by("-sales_units", "name")[:10]
    return render(request, "admin/control/inner/store_report.html", {
        "vendor": vendor,
        "report": {"products": vendor.products.count(), "active_products": vendor.products.filter(is_published=True).count(), "orders": orders.count(), "gross": sales["gross"] or 0, "commission": sales["commission"] or 0, "net": sales["net"] or 0},
...
def control_product_delete(request, product_id):
    ...
    try:
        ...
        messages.error(request, "لا يمكن حذف المنتج لوجود طلبات مرتبطة به.")
    return render_control_partial(request, "products") if is_ajax(request) else redirect(CONTROL_PRODUCTS)
...
def control_product_report(request, product_id):
    product = get_object_or_404(Product.objects.select_related("vendor").prefetch_related("categories"), pk=product_id)
    sales = OrderItem.objects.filter(product=product).aggregate(quantity=Sum("quantity"), revenue=Sum("vendor_total"))
    return render(request, "admin/control/inner/product_report.html", {
        "product": product,
        "report": {"units_sold": sales["quantity"] or 0, "revenue": sales["revenue"] or 0, "orders": OrderItem.objects.filter(product=product).values("order_id").distinct().count(), "stock": product.available_stock, "rating": product.rating, "reviews": product.reviews_count, "published": product.is_published},
    })
