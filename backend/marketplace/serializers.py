...
class ProductVariantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "sku", "color", "size", "price_override", "available_stock", "stock", "reserved_stock", "is_active", "effective_price"]
        read_only_fields = ["available_stock", "reserved_stock", "effective_price"]
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ["id", "vendor", "categories", "category_ids", "sku", "name", "slug", "description", "brand", "material", "shipping_note", "return_policy", "price", "sale_price", "effective_price", "discount_percent", "currency", "stock", "reserved_stock", "available_stock", "colors", "sizes", "hashtags", "details", "main_image_url", "images", "gallery", "image_data_urls", "main_image_data_url", "keep_image_ids", "delete_image_ids", "variants", "rating", "reviews_count", "sold_count", "is_published", "is_trending"]
        read_only_fields = ["id", "vendor", "reserved_stock", "available_stock", "effective_price", "discount_percent", "main_image_url", "gallery", "rating", "reviews_count", "sold_count"]
    ...
    def _validate_variant_rows(self, rows, instance=None):
        seen_dimensions = set()
        seen_skus = set()
        existing_by_id = {variant.id: variant for variant in instance.variants.all()} if instance else {}
        for row in rows:
            variant_id = row.get("id")
            if variant_id and variant_id not in existing_by_id:
                raise serializers.ValidationError({"variants": f"الخيار {variant_id} لا ينتمي إلى هذا المنتج."})
        ...
        if variants_data is not None:
            ...
            for row in variants_data:
                ...
                if variant_id:
                    ...
                    variant = ProductVariant.objects.create(product=product, is_active=True, **row)
