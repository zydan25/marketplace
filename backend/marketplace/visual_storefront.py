...
def visual_editor(request):
    ...
    catalog = {
        ...
        "products": [
            {"id": p.id, "name": p.name, "vendor": p.vendor.store_name}
            for p in products_qs[:500]
        ],
    }
    return render(
        request,
        "admin/marketplace/storefront_builder_v6.html",
        {
            "sections": sections,
...
def update_section(request, pk):
    ...
    try:
        data = (
            json.loads(request.POST.get("payload", "{}"))
    ...
    if action == "duplicate":
        ...
        for item in StorefrontSection.objects.filter(
            vendor=section.vendor, sort_order__gte=new_order
        ).order_by("-sort_order"):
            item.sort_order += 1
            item.save(update_fields=["sort_order", "updated_at"])
        clone = StorefrontSection.objects.create(
            owner=request.user,
            vendor=section.vendor,
            title=f"{section.title} — نسخة",
    ...
    if action in {"publish", "unpublish"}:
        ...
        data_config = config(section)
        data_config["published"] = published
        section.config = data_config
    ...
    if kind in {"product_grid", "trend"} and section_config.get("source") == "category":
        ...
        if not Category.objects.filter(id=section_config["source_category_id"], is_active=True).exists():
    ...
    for field_name, url_key, path_key in (
        ...
        ("mobile_image", "mobile_image_url", "mobile_image_path"),
    ):
...
def reorder_sections(request):
    ...
    rows = {
        item.id: item
        for item in StorefrontSection.objects.filter(id__in=ids).select_related("vendor")
    }
    if set(ids) != set(rows):
