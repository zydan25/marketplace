...
class WalletOrderV2ViewSet(SecureOrderV2ViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        address=order.shipping_address or {}; city_id=address.get("city_id")
        ...
        for vendor_order in order.vendor_orders.select_for_update().select_related("vendor"):
            fee=Decimal("0.00")
            if city:
                rule=VendorCityShipping.objects.filter(vendor=vendor_order.vendor,city=city,is_active=True).first()
                fee=rule.fee if rule else city.shipping_fee
            vendor_total=max(Decimal("0.00"),vendor_order.subtotal-vendor_order.discount+fee)
            vendor_order.shipping_fee=fee; vendor_order.total=vendor_total; vendor_order.vendor_net=vendor_order.subtotal-vendor_order.commission+fee
            vendor_order.save(update_fields=["shipping_fee","total","vendor_net","updated_at"])
            vendor_shipping_total+=fee
        order.shipping_fee=vendor_shipping_total+office_fee
        order.total=max(Decimal("0.00"),order.subtotal-order.discount+order.shipping_fee)
        metadata=dict(order.metadata or {}); metadata["shipping_source"]="vendor_city"; metadata["office_fee"]=str(office_fee); metadata["vendor_shipping_fee"]=str(vendor_shipping_total); order.metadata=metadata
        order.save(update_fields=["shipping_fee","total","metadata","updated_at"])
        ...
        hold=WalletHold.objects.create(wallet=wallet,order=order,amount=order.total,note="مبلغ معلق حتى تأكيد العميل واعتماد الإدارة",metadata={"office_fee":str(office_fee),"vendor_shipping_fee":str(vendor_shipping_total)})
    ...
    def confirm_received(self,request,pk=None):
        ...
        metadata=dict(order.metadata or {}); metadata["customer_confirmed"]=True; metadata["confirmed_at"]=timezone.now().isoformat(); order.metadata=metadata; order.save(update_fields=["metadata","updated_at"])
    ...
    def admin_release(self,request,pk=None):
        ...
        metadata=dict(order.metadata or {}); metadata["admin_released"]=True; metadata["released_at"]=timezone.now().isoformat(); order.metadata=metadata; order.payment_status="paid"; order.save(update_fields=["metadata","payment_status","updated_at"])
    ...
    def reject_item(self,request,pk=None):
        ...
        decision.status=OrderItemDecision.Status.REJECTED; decision.reason=reason; decision.decided_by=request.user; decision.save(update_fields=["status","reason","decided_by","updated_at"])
        hold=WalletHold.objects.select_for_update().filter(order=order).first()
        ...
        hold.refunded_amount+=refund; hold.status=WalletHold.Status.REFUNDED if hold.refunded_amount>=hold.amount else WalletHold.Status.PARTIAL; hold.save(update_fields=["refunded_amount","status","updated_at"])
