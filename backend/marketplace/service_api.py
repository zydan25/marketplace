...
class ServiceViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        if not self._admin(): raise PermissionDenied("إدارة الخدمات للمدير فقط")
        serializer.save()
    def perform_update(self, serializer):
        if not self._admin(): raise PermissionDenied("إدارة الخدمات للمدير فقط")
        serializer.save()
...
class ServiceSubmissionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return qs if (self.request.user.is_staff or self.request.user.role == "admin") else qs.filter(customer=self.request.user)
    @transaction.atomic
