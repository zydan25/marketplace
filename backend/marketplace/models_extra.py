...
class WalletHold(TimeStampedModel):
    class Status(models.TextChoices):
        HELD = "held", "معلق"
        RELEASED = "released", "مطلق"
        REFUNDED = "refunded", "مسترد"
        PARTIAL = "partial", "مسترد جزئيًا"
...
class Service(TimeStampedModel):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            base = slugify(self.name, allow_unicode=True) or "service"
            candidate = base
            counter = 2
...
class ServiceField(TimeStampedModel):
    class FieldTypes(models.TextChoices):
        TEXT = "text", "نص"
        TEXTAREA = "textarea", "وصف"
        NUMBER = "number", "رقم"
        PHONE = "phone", "هاتف"
        DATE = "date", "تاريخ"
        SELECT = "select", "اختيار"
        MULTISELECT = "multiselect", "اختيار متعدد"
        IMAGE = "image", "رفع صورة"
        FILE = "file", "رفع ملف"
        CHECKBOX = "checkbox", "مربع اختيار"

    service = models.ForeignKey(Service, on_delete=models.CASCADE, related_name="fields")
    key = models.SlugField(max_length=100)
    label = models.CharField(max_length=180)
    field_type = models.CharField(max_length=30, choices=FieldTypes.choices, default=FieldTypes.TEXT)
