...
from .marketplace_models import Payment, VendorOrder

class WalletOrderViewSet(SecureOrderViewSet):
    @transaction.atomic
    def create(self, request, *args, **kwargs):
        if str(request.data.get("payment_method", "wallet")).lower() not in {"wallet", "balance"}:
            raise ValidationError({"payment_method": "الدفع في المتجر يتم من رصيد العميل فقط."})
        ...
        return Response(type(response.data)(response.data), status=response.status_code, headers=response.headers)
    ...
    def confirm_received(self, request, pk=None):
        ...
        order.metadata = {**(order.metadata or {}), "customer_confirmed": True, "confirmed_at": timezone.now().isoformat()}
    ...
    def admin_release(self, request, pk=None):
        ...
        if remaining <= 0: return Response({"success": True, "status": hold.status})
        ...
        order.metadata = {**(order.metadata or {}), "admin_released": True, "released_at": timezone.now().isoformat()}
        order.status = "delivered" if order.status != "cancelled" else order.status
        order.payment_status = "paid"
        order.save(update_fields=["metadata", "status", "payment_status", "updated_at"])
        OrderStatusHistory.objects.create(order=order, old_status=order.status, new_status=order.status, changed_by=request.user, note="إطلاق مبلغ الضمان للتجار")
    ...
    def reject_item(self, request, pk=None):
        ...
        if not str(request.data.get("reason", "")).strip(): raise ValidationError({"reason": "سبب الرفض مطلوب."})
        ...
        decision.status = OrderItemDecision.Status.REJECTED; decision.reason = str(request.data["reason"]).strip(); decision.decided_by = request.user; decision.save()
        refund = item.vendor_total
        hold = WalletHold.objects.select_for_update().get(order=order)
        if hold.refunded_amount + refund > hold.amount: refund = max(Decimal("0.00"), hold.amount - hold.refunded_amount)
        wallet = Wallet.objects.select_for_update().get(user=order.customer)
        wallet.balance += refund; wallet.save(update_fields=["balance", "updated_at"])
        ...
        hold.status = WalletHold.Status.PARTIAL if hold.refunded_amount < hold.amount else WalletHold.Status.REFUNDED
        hold.save(update_fields=["refunded_amount", "status", "updated_at"])
        return Response({"success": True, "order_item_id": item.id, "status": "rejected", "refund": str(refund), "reason": decision.reason})
