...
from .models import Category, Coupon, DesignTheme, Notification, Order, Product, StorefrontSection, User, VendorProfile, Wallet
from .models_extended import City, PriceGroup, ProductVariant
from .marketplace_models import Payment, Shipment, VendorApplication, VendorLedgerEntry, VendorOrder, VendorPayout
...
class DashboardForm(forms.ModelForm):
    class Meta:
        widgets = {"config": JsonTextarea(), "settings": JsonTextarea(), "tokens": JsonTextarea(), "layout": JsonTextarea(), "sections": JsonTextarea(), "details": JsonTextarea(), "colors": forms.TextInput(), "sizes": forms.TextInput(), "hashtags": forms.TextInput(), "shipping_address": JsonTextarea(), "metadata": JsonTextarea(), "audience": JsonTextarea()}
...
def _model_queryset(resource, request):
    model, _, _, _ = RESOURCE[resource]
...
def resource_list(request, resource):
    if resource not in RESOURCE:
        raise Http404
    model, form_class, columns, label = RESOURCE[resource]
    qs = _model_queryset(resource, request)
    q = request.GET.get("q", "").strip()
    if q:
        conditions = []
        for field in model._meta.fields:
            if field.get_internal_type() in {"CharField", "TextField", "EmailField", "SlugField"}:
                conditions.append(Q(**{f"{field.name}__icontains": q}))
        if conditions:
            query = conditions[0]
            for condition in conditions[1:]:
                query |= condition
            qs = qs.filter(query)
    page_items = qs.order_by("-pk")[:100]
    return render(request, "admin/crud_list.html", {"resource": resource, "label": label, "columns": columns, "objects": page_items, "q": q})
