...
def resolve_products(request, vendor, config):
    ...
    if not ids and isinstance(config.get("products"), list):
        ids = [int(item.get("id")) for item in config.get("products") if isinstance(item, dict) and str(item.get("id", "")).isdigit()]
    ...
    columns = max(1, min(int(config.get("columns", 2) or 2), 6))
...
def normalize_section_config(request, section, vendor):
    ...
    if section.section_type in {"hero", "banner"} and not config.get("slides") and (config.get("image_url") or config.get("imageUrl")):
