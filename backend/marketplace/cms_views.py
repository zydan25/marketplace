...
from .models import Category, StorefrontSection, VendorProfile, Product, DesignTheme
...
class DynamicHomeView(APIView):
    ...
    def get(self,request,slug=None):
        ...
        if vendor:
            raw_sections=StorefrontSection.objects.filter(vendor=vendor,is_visible=True).order_by("sort_order","id"); categories=[]; media=list(StorefrontMedia.objects.filter(vendor=vendor,is_active=True).order_by("updated_at","id")); theme=DesignTheme.objects.filter(vendor=vendor,is_active=True).first()
        else:
            raw_sections=StorefrontSection.objects.filter(vendor__isnull=True,is_visible=True).order_by("sort_order","id"); categories=list(Category.objects.filter(is_active=True).order_by("sort_order","name")); media=list(StorefrontMedia.objects.filter(vendor__isnull=True,is_active=True).order_by("updated_at","id")); theme=DesignTheme.objects.filter(is_global=True,is_active=True).first()
        data=[]
        for section in [s for s in raw_sections if (s.config or {}).get("published",True)]:
            config=normalize_section_config(request,section)
            if section.section_type=="category" or (section.section_type=="product_grid" and config.get("show_category_circles")):
                ids=[]
            ...
            if section.section_type=="product_grid" and str(config.get("source","")).lower()=="services":
                ...
                config["cards"]=[{"id":item.id,"title":item.name,"subtitle":item.description,"price":str(item.price),"currency":item.currency,"url":f"/services/{item.slug}","imageUrl":absolute(request,item.image.url) if item.image else "","bannerUrl":absolute(request,item.banner.url) if item.banner else "","visible":True,"isActive":True,"sortOrder":i} for i,sid in enumerate(ids) if (item:=selected.get(sid))]
        ...
        theme_data=None
        if theme:
            theme_data={"id":theme.id,"name":theme.name,"tokens":theme.tokens or {},"layout":theme.layout or {},"sections":theme.sections or {}}
        return Response({"success":True,"data":data,"theme":theme_data})
