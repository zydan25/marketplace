...
def category_circles(request, categories):
    return [{
        ...
        "url": f"/collection?category={quote(category.slug)}", "route": f"/collection?category={quote(category.slug)}",
...
def resolve_products(request, vendor, config):
    ...
    columns = max(2, min(int(config.get("columns", 2) or 2), 4))
    pages = 3 if config.get("scroll", True) else 1
    limit = min(rows * columns * pages, 40)
...
def normalize_section_config(request, section, vendor):
    ...
    if section.section_type in {"product_grid", "trend"}:
        ...
        if config.get("show_categories") and vendor:
...
class DynamicHomeView(APIView):
    ...
    def get(self, request, slug=None):
        ...
        theme_obj=DesignTheme.objects.filter(vendor=vendor,is_active=True).order_by("-updated_at").first() if vendor else None
