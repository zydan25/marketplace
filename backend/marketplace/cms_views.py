...
def resolve_products(request, vendor, config):
    ...
    if source in {"selected", "manual"} and ids:
        preserved = {item.id: index for index, item in enumerate(ids)}
        ...
        qs = qs.filter(categories__id=int(category_id)).order_by("-created_at")
        rows = list(qs)
    elif source in {"category", "from_category"} and category_slug:
        qs = qs.filter(categories__slug=category_slug).order_by("-created_at")
        rows = list(qs)
        ...
        rows = list(qs.filter(sale_price__isnull=False).exclude(sale_price__gte=Q(price)).order_by("-updated_at", "-sold_count"))
    else:
        rows = list(qs.order_by("-created_at"))

    limit = max(1, min(int(config.get("limit", config.get("rows", 2) * config.get("columns", 2)) or 8), 100))
    if source not in {"manual", "selected"}:
        rows = rows[:limit]
    else:
        rows = rows[:limit]
    return [product_card(request, product) for product in rows]
...
def normalize_section_config(request, section, vendor):
        ...
        if custom:
            ...
            if ids:
                ...
                ordered = (
                    ...
                    if vendor
                    else list(Category.objects.filter(is_active=True).order_by("sort_order", "name"))
                )
    ...
    if section.section_type == "tab":
        config["tabs"] = sorted(
            [item for item in (config.get("tabs") or []) if isinstance(item, dict) and item.get("title")],
            key=lambda item: item.get("sortOrder", item.get("sort_order", 0)),
        )
...
class DynamicHomeView(APIView):
    ...
    def get(self, request, slug=None):
        ...
        if vendor:
            own_categories = Category.objects.filter(
                is_active=True,
                products__vendor=vendor,
                products__is_published=True,
            ).distinct().order_by("sort_order", "name")
                ...
                data.append(
                    {
                        ...
                        "sort_order": 10,
                        "is_visible": True,
                        "config": {"circles": category_circles(request, own_categories)},
                    }
                )
        else:
            ...
            if global_media and not any(item["type"] in {"hero", "banner"} for item in data):
                data.append(
                    {
                        "id": "system-media",
                        "type": "banner",
                        ...
                        "config": {
                            "slides": [
                                ...
                                for index, item in enumerate(global_media)
                            ]
                        },
                    }
                )

        theme_qs = DesignTheme.objects.filter(is_active=True)
        if vendor:
            theme_obj = theme_qs.filter(vendor=vendor).order_by("-updated_at").first() or theme_qs.filter(is_global=True).order_by("-updated_at").first()
        else:
            theme_obj = theme_qs.filter(is_global=True).order_by("-updated_at").first()
        theme = (
            {
                "id": theme_obj.id,
                "name": theme_obj.name,
                ...
                "sections": theme_obj.sections or [],
                "is_global": theme_obj.is_global,
            }
            if theme_obj
            else None
        )
