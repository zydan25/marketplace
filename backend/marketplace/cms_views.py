...
def category_circles(request, categories, image_map=None):
    ...
    return [
        {
            ...
            "imageUrl": absolute(request, image_map.get(category.id) or (category.image.url if category.image else "")),
            "visible": True,
            "isActive": True,
            "sortOrder": index,
        }
...
def custom_category_circles(request, config):
    ...
    for index, item in enumerate(config.get("category_circles") or []):
        ...
        circles.append({
            "id": item.get("id", f"custom-{index}"),
            "title": item.get("title"),
            "name": item.get("title"),
            "targetCategory": target,
            "categorySlug": target,
            "url": item.get("url", "") or (f"/collection?category={quote(str(target))}" if target else ""),
            "imageUrl": absolute(request, item.get("imageUrl") or item.get("image_url") or ""),
...
def normalize_section_config(request, section, vendor):
    ...
    config.setdefault("rows", 2)
    config.setdefault("columns", 4)
    config.setdefault("columns_desktop", config.get("columns", 4))
    config.setdefault("columns_tablet", 3)
    config.setdefault("columns_mobile", 2)
    config.setdefault("mobile_scroll", False)
    ...
    if section.section_type == "category":
        ...
        config["columns_desktop"] = int(config.get("columns_desktop") or 4)
        config["columns_tablet"] = int(config.get("columns_tablet") or 3)
        config["columns_mobile"] = int(config.get("columns_mobile") or 2)
    if section.section_type in {"product_grid", "trend"}:
        if section.section_type == "trend":
            config["source"] = "trending"
        config["columns"] = max(1, min(int(config.get("columns", config.get("columns_mobile", 2)) or 2), 6))
        config["rows"] = max(1, min(int(config.get("rows", 2) or 2), 20))
        config["limit"] = max(1, min(int(config.get("limit", config["rows"] * config["columns"]) or 8), 100))
        config["scroll"] = bool(config.get("scroll", config.get("mobile_scroll", False)))
        config["products"] = resolve_products(request, vendor, config)
        config["show_categories"] = bool(config.get("show_categories", False))
        if config["show_categories"]:
            if vendor:
                config["category_circles"] = vendor_categories(request, vendor)
            elif config.get("category_ids"):
                ids = [int(x) for x in config.get("category_ids", []) if str(x).isdigit()]
                cats = Category.objects.filter(id__in=ids, is_active=True).order_by("sort_order", "name")
                config["category_circles"] = category_circles(request, cats)
    if section.section_type == "tab":
        config["tabs"] = sorted([item for item in (config.get("tabs") or []) if isinstance(item, dict) and item.get("title")], key=lambda item: item.get("sortOrder", item.get("sort_order", 0)))
    return config


def materialize_theme_sections(request, theme, vendor=None, fallback_sections=None):
    sections = []
    raw = theme.sections if theme and isinstance(theme.sections, list) else []
    global_media = list(StorefrontMedia.objects.filter(vendor__isnull=True, is_active=True).order_by("sort_order", "id")) if not vendor else []
    categories = list(Category.objects.filter(is_active=True).order_by("sort_order", "name"))
    products = None
    for index, item in enumerate(raw):
        if not isinstance(item, dict) or item.get("enabled", item.get("is_visible", True)) is False:
            continue
        config = dict(item.get("config") or {})
        key = str(item.get("key") or item.get("type") or f"section-{index}")
        section_type = str(item.get("type") or item.get("section_type") or "product_grid")
        title = str(item.get("title") or config.get("title") or "")
        order = int(item.get("sort_order", item.get("sortOrder", index + 1)) or index + 1)
        if section_type == "hero" and not config.get("slides") and global_media:
            config["slides"] = [
                {"id": media.id, "title": media.name, "subtitle": media.alt_text, "ctaLabel": "استكشف الآن" if media.target_url else "", "url": media.target_url or "", "imageUrl": absolute(request, media.image.url) if media.image else "", "visible": True, "isActive": True, "sortOrder": media.sort_order}
                for media in global_media
            ]
        if section_type == "category_grid":
            selected_ids = [int(value) for value in config.get("category_ids", []) if str(value).isdigit()]
            ordered = [category for category in categories if not selected_ids or category.id in selected_ids]
            if selected_ids:
                order_map = {value: idx for idx, value in enumerate(selected_ids)}
                ordered.sort(key=lambda category: order_map.get(category.id, 10**9))
            config["circles"] = category_circles(request, ordered[: max(1, int(config.get("rows", 3))) * max(1, int(config.get("columns", 4)))])
        elif section_type == "brand_grid":
            source_qs = Product.objects.filter(is_published=True, vendor__status="active")
            if vendor:
                source_qs = source_qs.filter(vendor=vendor)
            rows = source_qs.exclude(brand="").values("brand").order_by("brand")
            brand_names = [row["brand"] for row in rows if row["brand"]]
            brand_cards = []
            seen = set()
            for brand in brand_names:
                if brand in seen:
                    continue
                seen.add(brand)
                sample = source_qs.filter(brand=brand).order_by("-updated_at").first()
                brand_cards.append({"id": brand, "title": brand, "targetCategory": "", "url": f"/collection?brand={quote(brand)}", "imageUrl": absolute(request, sample.main_image.url) if sample and sample.main_image else "", "visible": True, "isActive": True, "sortOrder": len(brand_cards)})
                if len(brand_cards) >= int(config.get("limit", 8) or 8):
                    break
            config["circles"] = brand_cards
        elif section_type == "product_grid":
            config["products"] = resolve_products(request, vendor, config)
        elif section_type == "catalog_toolbar":
            config.setdefault("count", len(products or resolve_products(request, vendor, {"source": "latest", "limit": 100})))
        elif section_type == "promo_strip":
            config.setdefault("items", [])
        elif section_type == "notice":
            config.setdefault("text", "")
        elif section_type == "tabs":
            config.setdefault("tabs", config.get("items") or [])
        elif section_type == "header":
            config.setdefault("category_chips", [item for item in category_circles(request, categories)[: int(config.get("category_chip_limit", 6) or 6)]])
        elif section_type == "bottom_nav":
            config.setdefault("items", [])
        sections.append({"id": key, "key": key, "type": section_type, "title": title, "sort_order": order, "is_visible": True, "config": config})
    if not sections and fallback_sections:
        sections = fallback_sections
    return sorted(sections, key=lambda item: (item.get("sort_order", 0), str(item.get("id", ""))))
...
class DynamicHomeView(APIView):
    ...
    def get(self, request, slug=None):
        ...
        raw_sections = StorefrontSection.objects.filter(vendor=vendor, is_visible=True).order_by("sort_order", "id") if vendor else StorefrontSection.objects.filter(vendor__isnull=True, is_visible=True).order_by("sort_order", "id")
        fallback_data = []
        for section in raw_sections:
            config = section.config or {}
            if config.get("published", True):
                fallback_data.append({"id": section.id, "type": section.section_type, "title": section.title, "sort_order": section.sort_order, "is_visible": section.is_visible, "config": normalize_section_config(request, section, vendor)})
        if vendor:
            ...
            if not any(item["type"] == "category" for item in fallback_data):
                fallback_data.append({"id": "system-categories", "type": "category", "title": "الفئات", "sort_order": 10, "is_visible": True, "config": {"circles": category_circles(request, own_categories)}})
        else:
            categories = list(Category.objects.filter(is_active=True).order_by("sort_order", "name"))
            if categories and not any(item["type"] == "category" for item in fallback_data):
                fallback_data.append({"id": "system-categories", "type": "category", "title": "الفئات", "sort_order": 10, "is_visible": True, "config": {"circles": category_circles(request, categories)}})

        if theme_obj:
            data = materialize_theme_sections(request, theme_obj, vendor=vendor, fallback_sections=fallback_data)
            theme = {"id": theme_obj.id, "name": theme_obj.name, "tokens": theme_obj.tokens or {}, "layout": theme_obj.layout or {}, "sections": data, "is_global": theme_obj.is_global}
        else:
            data = fallback_data
            theme = None
