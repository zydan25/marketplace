...
private fun OperationRow(service: ServiceDto, selected: Boolean, accent: Color, onClick: () -> Unit) {
    Surface(modifier = Modifier.fillMaxWidth().clickable(onClick = onClick), shape = RoundedCornerShape(15.dp), color = if (selected) accent.copy(alpha = .09f) else ServicesCard, border = BorderStroke(1.dp, if (selected) accent else Color(0xFFE8E6EC)), shadowElevation = if (selected) 2.dp else 0.dp) {
...
@Composable
private fun PaymentPackageCard(
    item: ServiceItemDto,
    selected: Boolean,
    accent: Color,
    ...
    val unavailable = item.availability["available"] == "false"
    Surface(
        modifier = Modifier.fillMaxWidth().clickable(enabled = !unavailable, onClick = onClick),
        shape = RoundedCornerShape(17.dp),
        color = if (selected) accent.copy(alpha = .08f) else Color(0xFFFCFBFD),
        border = BorderStroke(1.dp, if (selected) accent else Color(0xFFE8E4EA)),
        shadowElevation = if (selected) 2.dp else 0.dp
    ) {
        Column(Modifier.padding(11.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.Top, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                Box(
                    Modifier.size(34.dp).background(if (selected) accent else accent.copy(alpha = .10f), CircleShape),
                    contentAlignment = Alignment.Center
                ) {
                    Text("₿", color = if (selected) Color.White else accent, fontSize = 14.sp, fontWeight = FontWeight.ExtraBold)
                }
                Column(Modifier.weight(1f)) {
                    Text(item.name, fontSize = 11.sp, fontWeight = FontWeight.Bold, maxLines = 2)
                    Text(
                        if (unavailable) item.availability["reason"] ?: "غير متاح" else "اضغط لاختيار الباقة",
                        fontSize = 8.sp,
                        color = if (unavailable) Color(0xFFAC3C3C) else Color(0xFF85818B)
                    )
                }
                if (selected) Icon(Icons.Default.CheckCircle, null, tint = accent, modifier = Modifier.size(19.dp))
            }
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
                Text("${item.price?.let { formatMoney(it.toDoubleOrNull() ?: 0.0) } ?: "—"} ${item.currency}", color = accent, fontSize = 12.sp, fontWeight = FontWeight.ExtraBold)
                Surface(shape = RoundedCornerShape(9.dp), color = if (selected) accent else accent.copy(alpha = .10f)) {
                    Text(if (selected) "مختارة" else "اختيار", modifier = Modifier.padding(horizontal = 8.dp, vertical = 4.dp), color = if (selected) Color.White else accent, fontSize = 8.sp, fontWeight = FontWeight.Bold)
                }
            }
        }
    }
...
fun PaymentNetworkScreen(
                ...
                item {
                    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(20.dp), colors = CardDefaults.cardColors(containerColor = ServicesCard), elevation = CardDefaults.cardElevation(1.dp)) {
                        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                                    ...
                                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                        pair.forEach { item ->
                                            ...
                                            Box(Modifier.weight(1f)) {
                                                PaymentPackageCard(item, selected, accent, formatMoney) { selectedItem = item }
                                            }
                                        }
                            ...
                            Button(
                                onClick = {
                                    val isPayment = service.serviceKind != "query" && service.serviceKind != "catalog"
                                    if (isPayment) showConfirm = true else submit()
                                },
                                enabled = !submitting,
    ...
    if (showConfirm && selectedService != null) {
        val service = selectedService!!
        val selectedPackageName = selectedItem?.name ?: service.name
        val selectedAmount = selectedItem?.price?.toDoubleOrNull()?.let { formatMoney(it) }
            ?: values.entries.firstOrNull { it.key.lowercase() in setOf("amount", "price", "value") }?.value
            ?: "غير محدد"
        AlertDialog(
            onDismissRequest = { if (!submitting) showConfirm = false },
            title = { Text("تأكيد التسديد", fontWeight = FontWeight.ExtraBold) },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(7.dp)) {
                    Text("راجع تفاصيل العملية قبل الإرسال.", fontSize = 11.sp, color = Color(0xFF6F6B74))
                    Surface(shape = RoundedCornerShape(14.dp), color = accent.copy(alpha = .07f)) {
                        Column(Modifier.fillMaxWidth().padding(11.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) {
                            Text("المزود: ${selectedProvider?.name.orEmpty()}", fontSize = 10.sp)
                            Text("الخدمة: ${service.name}", fontSize = 10.sp)
                            Text("الرقم: ${phone.ifBlank { values["mobile"].orEmpty() }}", fontSize = 10.sp)
                            Text("الباقة / الفئة: $selectedPackageName", fontSize = 10.sp, fontWeight = FontWeight.Bold)
                            Text("المبلغ: $selectedAmount", fontSize = 11.sp, fontWeight = FontWeight.ExtraBold, color = accent)
                        }
                    }
                    Text("بعد التأكيد سيتم إرسال العملية إلى مزود الخدمة.", fontSize = 9.sp, color = Color(0xFF7A7680))
                }
            },
            confirmButton = {
                Button(onClick = { showConfirm = false; submit() }, enabled = !submitting, shape = RoundedCornerShape(11.dp)) {
                    Text("تأكيد التسديد", fontWeight = FontWeight.Bold)
                }
            },
            dismissButton = {
                TextButton(onClick = { showConfirm = false }, enabled = !submitting) { Text("مراجعة لاحقًا") }
            }
        )
    }
}
