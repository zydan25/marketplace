...
export default function OrderDetailsScreen() {
  ...
  const [orderState, setOrder] = useState<StoreOrder | null>(null);
  ...
  if (!isAuthenticated) {
    return <ScreenContainer><View style={s.center}><Text>سجّل الدخول لعرض الطلب.</Text></View></ScreenContainer>;
  }
  if (loading || !orderState) {
    return <ScreenContainer><View style={s.center}><ActivityIndicator color="#E60023"/><Text style={s.muted}>جارٍ تحميل تفاصيل الطلب...</Text></View></ScreenContainer>;
  }
  ...
  return (
    <ScreenContainer className="bg-[#F6F6F6]" edges={["top", "bottom", "left", "right"]}>
      <View style={s.header}>
        <TouchableOpacity onPress={() => router.back()}><MaterialIcons name="arrow-forward" size={24} color="#111" /></TouchableOpacity>
        <View style={s.headerCopy}><Text style={s.title}>تفاصيل الطلب</Text><Text style={s.code}>{order.orderCode} · {order.statusLabel}</Text></View>
        <TouchableOpacity onPress={() => router.push(`/order-chat/${order.id}` as never)}><MaterialIcons name="chat" size={22} color="#111" /></TouchableOpacity>
      </View>

      <ScrollView contentContainerStyle={s.page}>
        <View style={s.timeline}>
          <Text style={s.sectionTitle}>حالة الطلب</Text>
          {(order.timeline ?? []).length ? order.timeline!.map((event, index) => (
            <View key={`${event.createdAt}-${index}`} style={s.timelineRow}>
              <View style={s.dot}/>
              <View style={s.timelineCopy}>
                <Text style={s.timelineTitle}>{label(event.status)}</Text>
                <Text style={s.timelineTime}>{new Date(event.createdAt).toLocaleString("ar-YE")}</Text>
                {event.note ? <Text style={s.timelineNote}>{event.note}</Text> : null}
              </View>
            </View>
          )) : <View style={s.currentStatus}><Text style={s.currentStatusText}>{order.statusLabel}</Text></View>}
        </View>

        <View style={s.card}>
          <View style={s.cardHeader}>
            <Text style={s.sectionTitle}>المنتجات ({order.items.length})</Text>
            {pending && isCustomer && !editing ? <TouchableOpacity onPress={() => setEditing(true)}><Text style={s.link}>تعديل الطلب</Text></TouchableOpacity> : editing ? <TouchableOpacity onPress={saveEdit} disabled={saving}><Text style={s.link}>حفظ</Text></TouchableOpacity> : null}
          </View>
          {order.items.map((item) => (
            <View key={item.id} style={s.item}>
              <TouchableOpacity style={s.imageBox} onPress={() => router.push(`/product/${item.productId}` as never)}>
                {item.imageUrl ? <Image source={{uri:item.imageUrl}} style={s.image}/> : <MaterialIcons name="image" size={24} color="#AAA"/>}
              </TouchableOpacity>
              <View style={s.itemCopy}>
                <TouchableOpacity onPress={() => router.push(`/product/${item.productId}` as never)}><Text style={s.itemName}>{item.productName}</Text></TouchableOpacity>
                <Text style={s.meta}>اللون: {item.color || "غير محدد"} · المقاس: {item.size || "غير محدد"}</Text>
                <Text style={s.meta}>SKU: {item.sku}</Text>
                {editing ? (
                  <View style={s.qty}>
                    <TouchableOpacity onPress={() => setQuantities((q) => ({...q,[item.id]:Math.max(1,(q[item.id]??item.quantity)-1)}))}><MaterialIcons name="remove" size={18}/></TouchableOpacity>
                    <Text style={s.qtyText}>{quantities[item.id] ?? item.quantity}</Text>
                    <TouchableOpacity onPress={() => setQuantities((q) => ({...q,[item.id]:(q[item.id]??item.quantity)+1}))}><MaterialIcons name="add" size={18}/></TouchableOpacity>
                  </View>
                ) : <Text style={s.qtyText}>الكمية: {item.quantity}</Text>}
              </View>
              <View style={s.itemRight}>
                <Text style={s.itemPrice}>{(item.unitPrice*item.quantity).toLocaleString("ar-YE")} {order.currency}</Text>
                {isCustomer && order.status === "delivered" && escrow && !escrow.customer_confirmed ? <TouchableOpacity onPress={() => setRejecting(rejecting === item.id ? null : item.id)} style={s.rejectButton}><Text style={s.rejectText}>اعتراض</Text></TouchableOpacity> : null}
                {disputedItems.has(item.id) ? <Text style={s.dispute}>اعتراض معلّق</Text> : null}
              </View>
            </View>
          ))}
          {rejecting !== null ? (
            <View style={s.reasonBox}>
              <Text style={s.reasonTitle}>سبب الاعتراض على القطعة</Text>
              <TextInput value={reason} onChangeText={setReason} style={s.reasonInput} placeholder="اذكر ما لا يطابق الطلب" placeholderTextColor="#999" multiline textAlign="right"/>
              <View style={s.reasonActions}><TouchableOpacity onPress={() => setRejecting(null)} style={s.cancelButton}><Text>إلغاء</Text></TouchableOpacity><TouchableOpacity onPress={() => rejectItem(rejecting)} disabled={saving} style={s.dangerButton}><Text style={s.dangerText}>إرسال الاعتراض</Text></TouchableOpacity></View>
            </View>
          ) : null}
        </View>

        <View style={s.card}>
          <Text style={s.sectionTitle}>العنوان والتوصيل</Text>
          <Text style={s.address}>{String(order.shippingAddress?.title ?? "")}</Text>
          <Text style={s.address}>{String(order.shippingAddress?.district ?? "")} · {String(order.shippingAddress?.street ?? "")}</Text>
          <View style={s.moneyRow}><Text style={s.moneyLabel}>رسوم التوصيل</Text><Text style={s.moneyValue}>{order.shippingFee.toLocaleString("ar-YE")} {order.currency}</Text></View>
        </View>

        <View style={s.card}>
          <Text style={s.sectionTitle}>الملخص المالي</Text>
          <Row label="المجموع الفرعي" value={order.subtotal}/><Row label="الخصم" value={-order.discount}/><Row label="الشحن" value={order.shippingFee}/>
          <View style={s.divider}/><Row label="الإجمالي" value={order.totalAmount} strong/>
          <View style={s.holdBox}>{escrow ? <><MaterialIcons name="lock-clock" size={20} color="#A96A00"/><Text style={s.holdText}>المبلغ المحجوز: {escrow.held_amount} {order.currency} · المتاح للتاجر لا يطلق حتى انتهاء المراجعة.</Text></> : <Text style={s.meta}>حالة الدفع: {order.paymentStatus}</Text>}</View>
        </View>
        ...
        {admin && Object.entries(escrow?.disputes ?? {}).filter(([, dispute]) => dispute.status === "pending").map(([key, dispute]) => (
          <View key={key} style={s.adminDispute}>
            <Text style={s.sectionTitle}>اعتراض القطعة #{key}</Text>
            <Text style={s.meta}>{dispute.reason}</Text>
            <View style={s.reasonActions}><TouchableOpacity disabled={saving} onPress={() => resolve(Number(key), "refund")} style={s.dangerButton}><Text style={s.dangerText}>استرداد للعميل</Text></TouchableOpacity><TouchableOpacity disabled={saving} onPress={() => resolve(Number(key), "release")} style={s.okButton}><Text style={s.okText}>إطلاق للتاجر</Text></TouchableOpacity></View>
          </View>
        ))}
