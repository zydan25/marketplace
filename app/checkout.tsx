...
export default function CheckoutScreen(){
  const insets = useSafeAreaInsets(); const {items,removeItem,validateCartWithServer}=useCart(); const {lines}=useLocalSearchParams<{lines?:string}>();
  const selectedIds=useMemo(()=>new Set((lines??"").split(",").filter(Boolean)),[lines]); const orderItems=selectedIds.size?items.filter(i=>selectedIds.has(i.lineId)):items;
  ...
  useEffect(()=>{djangoApi<{results?:Address[]}|Address[]>("/api/addresses/").then(data=>{const list=Array.isArray(data)?data:(data.results??[]);setAddresses(list);setSelectedAddressId(list.find(x=>x.is_default)?.id??list[0]?.id??null)}).catch(()=>setAddresses([])).finally(()=>setLoadingAddresses(false));},[]);
  ...
  const subtotal=Number(serverData?.subtotal??orderItems.reduce((t,i)=>t+i.unitPrice*i.quantity,0)), shipping=Number(serverData?.shipping_fee??0), discount=Number(serverData?.discount??0), total=Number(serverData?.total??subtotal+shipping-discount); const selectedAddress=addresses.find(x=>x.id===selectedAddressId);
  async function submit(){if(!orderItems.length)return Alert.alert("الحقيبة فارغة","أضف منتجًا واحدًا على الأقل قبل إكمال الطلب.");if(!selectedAddress)return Alert.alert("العنوان مطلوب","اختر عنوان التوصيل قبل تأكيد الطلب.");try{setSubmitting(true);const validation=await validateCartWithServer(selectedAddress.city?.id,couponCode);setServerData(validation);if(validation?.valid===false){Alert.alert("تحديث في السلة",validation.errors?.join("\n")||"تغيرت بعض المنتجات أو الأسعار.");return;}const order=await createOrder(orderItems.map(item=>({productId:Number(item.product.id),variantId:item.variantId,color:item.color,size:item.size,quantity:item.quantity})),{address_id:selectedAddress.id,city_id:selectedAddress.city?.id,title:selectedAddress.title,district:selectedAddress.district,street:selectedAddress.street,phone:selectedAddress.phone},"YER",couponCode);orderItems.forEach(item=>removeItem(item.lineId));router.replace(`/order-chat/${order.id}` as never);}catch(error){Alert.alert("تعذر إنشاء الطلب",error instanceof Error?error.message:"سجّل الدخول ثم حاول مرة أخرى.");}finally{setSubmitting(false);}}
    ...
    return (
      <ScreenContainer edges={["top","bottom","left","right"]} className="bg-[#F6F6F6]">
        ...
        <View style={{flex: 1, justifyContent: "center", alignItems: "center"}}>
          <Text style={{color: "#777", fontSize: 16}}>الحقيبة فارغة</Text>
        </View>
      </ScreenContainer>
    );
  }
  ...
  return (
    <ScreenContainer edges={["top","bottom","left","right"]} className="bg-[#F6F6F6]">
      <View style={styles.header}>
        <TouchableOpacity onPress={()=>router.back()}><MaterialIcons name="close" size={24} color="#171717"/></TouchableOpacity>
        <Text style={styles.headerTitle}>مراجعة الطلب</Text>
        <View style={{width:24}}/>
      </View>
      <View style={{flex: 1}}>
        <ScrollView style={styles.scroll} contentContainerStyle={[styles.content,{paddingBottom: 20}]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator automaticallyAdjustKeyboardInsets>
        ...
        <View style={[styles.bottom,{paddingBottom:Math.max(insets.bottom,10)}]}>
          <TouchableOpacity style={[styles.submit,submitting&&styles.submitDisabled]} disabled={submitting} onPress={submit}>
            <Text style={styles.submitText}>{submitting?"جارٍ إنشاء الطلب...":`تأكيد الطلب · ${formatYER(total)}`}</Text>
