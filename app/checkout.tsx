...
export default function CheckoutScreen() {
  ...
  useEffect(() => {
    djangoApi<{ results?: Address[] } | Address[]>("/api/addresses/")
      .then((data) => {
        ...
        setAddresses(list);
        setSelectedAddressId(list.find((item) => item.is_default)?.id ?? list[0]?.id ?? null);
      })
      .catch(() => setAddresses([]))
  ...
  useEffect(() => {
    ...
    const cityId = addresses.find((item) => item.id === selectedAddressId)?.city?.id;
    validateCartWithServer(cityId, couponCode).then((result) => setServerData(result)).catch(() => setServerData(null));
  }, [orderItems.length, selectedAddressId, addresses, couponCode, validateCartWithServer]);

  const subtotal = Number(serverData?.subtotal ?? orderItems.reduce((total, item) => total + item.unitPrice * item.quantity, 0));
  ...
  const submit = async () => {
    if (!orderItems.length) {
      Alert.alert("الحقيبة فارغة", "أضيفي منتجًا واحدًا على الأقل قبل إكمال الطلب.");
      return;
    }
    if (!selectedAddress) {
      Alert.alert("العنوان مطلوب", "اختاري عنوان التوصيل قبل تأكيد الطلب.");
      return;
    }
    try {
      setSubmitting(true);
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: "#FFF", padding: 16, marginBottom: 12, borderRadius: 12, alignItems: "flex-end" },
  cardTitle: { color: "#111", fontSize: 15, fontWeight: "900", marginBottom: 14 },
  rowBetween: { width: "100%", flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  link: { color: "#E60023", fontSize: 11, fontWeight: "800" },
  ...
  address: { width: "100%", flexDirection: "row-reverse", gap: 10, padding: 12, borderWidth: 1, borderColor: "#EEE", borderRadius: 10, marginBottom: 8, alignItems: "center" },
  addressSelected: { borderColor: "#111", backgroundColor: "#FAFAFA" },
  addressRadio: { width: 24, height: 24, borderRadius: 12, backgroundColor: "#111", alignItems: "center", justifyContent: "center" },
  addressCopy: { flex: 1, alignItems: "flex-end" },
  addressTitle: { fontSize: 13, fontWeight: "900", color: "#111" },
  addressText: { fontSize: 11, color: "#666", marginTop: 3, textAlign: "right" },
  emptyAddress: { color: "#777", fontSize: 12, textAlign: "right" },
  couponRow: { width: "100%", flexDirection: "row-reverse", gap: 8 },
  couponInput: { flex: 1, height: 44, borderWidth: 1, borderColor: "#EEE", borderRadius: 10, paddingHorizontal: 12, textAlign: "right", fontSize: 13, backgroundColor: "#FAFAFA" },
  couponButton: { height: 44, paddingHorizontal: 16, borderRadius: 10, backgroundColor: "#111", justifyContent: "center" },
  couponButtonText: { color: "#FFF", fontSize: 12, fontWeight: "800" },
  summaryRow: { width: "100%", flexDirection: "row-reverse", justifyContent: "space-between", paddingVertical: 5 },
  summaryValue: { fontSize: 13, fontWeight: "700", color: "#333" },
  summaryLabel: { fontSize: 13, color: "#666" },
  totalValue: { fontSize: 19, color: "#E60023", fontWeight: "900" },
  totalLabel: { fontSize: 14, color: "#111", fontWeight: "900" },
  divider: { width: "100%", height: 1, backgroundColor: "#F0F0F0", marginVertical: 7 },
  method: { width: "100%", flexDirection: "row-reverse", gap: 12 },
