...
export default function ProfileScreen() { const {user,isAuthenticated,logout}=useAuth(); const [walletBalance,setWalletBalance]=useState("0"); useEffect(() => { if (user?.role === "vendor") router.replace("/vendor" as never); }, [user?.role]); useEffect(() => { if (!isAuthenticated || user?.role !== "customer") return; ApiClient.get<any>("/api/wallets/").then((response) => setWalletBalance(String(response.results?.[0]?.balance ?? response.balance ?? "0"))).catch(() => undefined); }, [isAuthenticated,user?.role]); if(user?.role === "vendor") return null; if(!isAuthenticated) return <Guest/>; const name=user?.name?.trim()||"شبيك"; const admin=user?.role==="admin"; return <ScreenContainer edges={["top","left","right"]} className="bg-[#F5F5F5]"><ScrollView style={{ flex: 1 }} contentContainerStyle={s.content} showsVerticalScrollIndicator={true}><View style={s.head}><TouchableOpacity onPress={()=>router.push("/settings" as never)}><MaterialIcons name="settings" size={27}/></TouchableOpacity><TouchableOpacity onPress={()=>later("رمز حسابي")}><MaterialIcons name="qr-code-scanner" size={27}/></TouchableOpacity><View style={s.identity}><View style={s.avatar}><Text style={s.avatarText}>{name.slice(0,1)}</Text></View><View><Text style={s.name}>{name}</Text><Text style={s.personal}>ملفي الشخصي <MaterialIcons name="edit-note" size={13}/></Text></View></View></View><View style={s.wallet}><Metric icon="confirmation-number" label="كوبونات" value="0"/><Metric icon="stars" label="نقاط" value={String(user?.points_balance ?? 0)}/><Metric icon="account-balance-wallet" label="محفظة" value={walletBalance ? `${walletBalance} ر.ي` : "0 ر.ي"} onPress={() => router.push("/customer-reports" as never)}/><Metric icon="card-giftcard" label="بطاقة هدية" value=""/><View style={s.coupon}><MaterialIcons name="local-offer" color="#E60023" size={20}/><Text style={s.couponText}>انتهز فرصة الكوبونات — ستظهر الخصومات المفعلة هنا</Text></View></View><Panel title="طلباتي" action="عرض الكل" onAction={()=>router.push("/orders" as never)}><Action icon="credit-card" label="غير مدفوع"/><Action icon="inventory-2" label="قيد التجهيز"/><Action icon="local-shipping" label="تم الشحن"/><Action icon="rate-review" label="تعليق"/></Panel><View style={s.panel}><Action icon="support-agent" label="تواصل معنا" onPress={()=>router.push("/support" as never)}/>
<Action icon="card-giftcard" label="إرسال هدية" onPress={()=>router.push("/extra-features" as never)}/>
<Action icon="analytics" label="تقارير الحساب" onPress={()=>router.push("/customer-reports" as never)}/><Action icon="storefront" label="التجار" onPress={()=>router.push("/vendors" as never)}/><Action icon="event-available" label="تسجيل الحضور"/><Action icon="verified-user" label="السياسة"/><Action icon="share" label="دعوة" onPress={()=>router.push("/invite" as never)}/></View><View style={s.metrics}><Text style={s.metric}>متابع{`\n`}<Text style={s.metricSmall}>0 متابع</Text></Text><Text style={s.metric}>تاريخ{`\n`}<Text style={s.metricSmall}>0 منتج</Text></Text><Text style={s.metric}>قائمة أمانياتي{`\n`}<Text style={s.metricSmall}>0 صنف</Text></Text></View><View style={s.favorites}><MaterialIcons name="favorite-border" size={34} color="#E60023"/><Text style={s.favTitle}>منتجاتك المفضلة ستظهر هنا</Text><Text style={s.favText}>بعد إضافة الفئات والأصناف، ستظهر لك العروض المناسبة.</Text></View>{admin?<TouchableOpacity style={s.admin} onPress={()=>router.push("/admin" as never)}><MaterialIcons name="admin-panel-settings" size={24} color="#F0B800"/><Text style={s.adminText}>لوحة التحكم بالإدارة</Text></TouchableOpacity>:null}<TouchableOpacity style={s.logout} onPress={logout}><MaterialIcons name="logout" size={22} color="#BB3F46"/><Text style={s.logoutText}>تسجيل الخروج</Text></TouchableOpacity></ScrollView></ScreenContainer>; }
