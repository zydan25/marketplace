...
export default function LoginScreen() {
  ...
  const submit = async () => {
    if (!phone.trim() || !password) return Alert.alert("بيانات ناقصة", "أدخل رقم الجوال وكلمة المرور.");
    try {
      setLoading(true);
...
function Field({ icon, ...props }: { icon: "phone-android" | "lock-outline"; value: string; onChangeText: (v: string) => void; placeholder: string; secureTextEntry?: boolean; keyboardType?: "phone-pad" }) { return <View style={s.field}><TextInput {...props} style={s.input} textAlign="right" placeholderTextColor="#999" autoCapitalize="none"/><MaterialIcons name={icon} size={20} color="#777" /></View>; }
