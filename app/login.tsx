...
export default function LoginScreen() {
  ...
  const submit = async () => {
    ...
    if (!value || !password) {
      Alert.alert("بيانات ناقصة", "أدخل اسم المستخدم أو رقم الهاتف وكلمة المرور.");
      return;
    }
    try {
      setLoading(true);
      const result = await djangoLogin(value, password);
      await Auth.setUserInfo(toAuthUser(result.user));
      const storedToken = await Auth.getSessionToken();
      if (!storedToken || storedToken !== result.token) throw new Error("تعذر حفظ جلسة الدخول. حاول مرة أخرى.");
  ...
  return <ScreenContainer edges={["top", "bottom"]} className="bg-[#FFF]">
    ...
    <ScrollView showsVerticalScrollIndicator={false} automaticallyAdjustKeyboardInsets keyboardShouldPersistTaps="handled" contentContainerStyle={s.content}>
      ...
      <Text style={s.sub}>استخدم اسم المستخدم أو رقم الهاتف للوصول إلى حسابك.</Text>
      <View style={s.form}>
        <Field icon="person-outline" value={identifier} onChangeText={setIdentifier} placeholder="اسم المستخدم أو رقم الهاتف" />
...
function Field({ icon, ...props }: { icon: "person-outline" | "lock-outline"; value: string; onChangeText: (v: string) => void; placeholder: string; secureTextEntry?: boolean }) {
