...
## النشر الدائم على Contabo

أصبح اسم المنصة في الكود **شبيك**، والعبارة الترحيبية هي: «شبيك لبيك، طلبك بين يديك». تم تجهيز ملفات النشر داخل مجلد `deploy/` للنطاق `shopik.alattab.site`. يستخدم Django/Gunicorn المنفذ الداخلي `127.0.0.1:5015` خلف Nginx، بينما تخدم Nginx واجهة Expo Web وملفات `/media/` و`/static/` وتوجه `/api/` و`/admin/` إلى Django.

التعليمات الكاملة موجودة في [`DEPLOY_SHABIK_AR.md`](./DEPLOY_SHABIK_AR.md)، وتشمل إعداد DNS، تثبيت Gunicorn، خدمة systemd، بناء واجهة الويب، إعداد Nginx، Certbot، الجدار الناري، النسخ الاحتياطية، وربط APK بالرابط الدائم. لا يمكن تنفيذ أوامر Contabo مباشرة من هذه الجلسة دون بيانات SSH أو وصول إلى الخادم، لذلك يجب تشغيل التعليمات على خادمك.

## المسار الموصى به للإنتاج

للاستخدام الفعلي، يجب تشغيل Django على استضافة دائمة مع PostgreSQL وHTTPS وتخزين للصور، ثم ضبط `DJANGO_SECRET_KEY` و`DJANGO_ALLOWED_HOSTS` و`CORS_ALLOWED_ORIGINS`. النسخة الحالية تعمل مع SQLite لتسهيل النقل الأولي، لكن لا يُنصح بها عند زيادة عدد الطلبات أو تشغيل أكثر من عامل Gunicorn. لا ينبغي استخدام الرابط المؤقت في الإنتاج.
